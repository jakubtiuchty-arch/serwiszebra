  ...
  useEffect(() => {
    async function fetchProduct() {
      try {
        ...
        setSlug(data.slug || '')
        setSku(data.sku || '')
        setProductType(data.product_type || 'glowica')
        setManufacturer(data.manufacturer || 'Zebra')
        setIsActive(data.is_active ?? true)
        
        setDeviceModel(data.device_model || '')
        setCompatibleModels(data.compatible_models || [])
        setResolutionDpi(data.resolution_dpi || null)
        
        // Capacity i additional params z attributes
        if (data.attributes) {
          setCapacity(data.attributes.capacity || '')
          // Konwertuj attributes na array dla additional params
          const params = Object.entries(data.attributes)
            .filter(([key]) => key !== 'capacity')
            .map(([key, value]) => ({ key, value: String(value) }))
          setAdditionalParams(params)
        }
        
        setPurchasePriceNetto(data.purchase_price_netto || 0)
        setMarginPercent(data.margin_percent || 25)
        setPriceNetto(data.price || 0)
        setVatRate(data.vat_rate || 23)
        setPriceBrutto(data.price_brutto || 0)
        
        setStock(data.stock || 0)
        setLeadTimeDays(data.lead_time_days || '1-3')
        ...
        // Ustaw preview istniejącego zdjęcia
        if (data.image_url) {
        ...
        alert('Błąd podczas ładowania produktu')
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    // Dodaj zdjęcie tylko jeśli użytkownik wybrał nowe
    if (imageFile) {
      formData.append('image', imageFile)
    }

    // Wyślij do API (PUT zamiast POST)
    const response = await fetch(`/api/admin/products/${productId}`, {
      method: 'PUT',
      body: formData
    })
    ...
    // Sukces - pokaż komunikat i przekieruj
    alert('✅ Produkt został zaktualizowany!')
    router.push('/admin/produkty')
    
  } catch (error: any) {
    console.error('Error updating product:', error)
    alert(error.message || 'Błąd podczas aktualizacji produktu')
  } finally {
    setSaving(false)
  }
}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">Ładowanie produktu...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">Modyfikuj dane produktu w katalogu</p>
      </div>
