...
export async function GET(
  ...
  try {
    ...
    console.log('📄 Pro Forma request for shop order:', orderId)
    ...
    const dueDate = new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toLocaleDateString('pl-PL')

    // Pobierz base URL z requestu
    const url = new URL(request.url)
    const baseUrl = `${url.protocol}//${url.host}`

    // Generuj wiersze tabeli dla produktów
...
<head>
  <meta charset="UTF-8">
  <title>Pro Forma ${shortId} - TAKMA Sklep</title>
  <style>
    @media print {
...
<body>
  <div class="print-controls no-print">
    <div>
      <strong>Pro Forma ${shortId}</strong> - Kliknij przycisk aby wydrukować lub zapisać jako PDF
    </div>
  ...
  <div class="document">
    ...
    <div class="bank-details">
      ...
      <div class="bank-row">
        ...
        <span class="bank-value">Zamówienie ${shortId}</span>
      </div>
    </div>
...
`
    ...
    console.error('❌ Error generating shop order proforma:', error)
