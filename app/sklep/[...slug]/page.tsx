...
const PRODUCT_TYPE_FAQ: Record<string, Array<{ question: string; answer: string }>> = {
  ...
  konwerter: [
    {
      question: 'Co zawiera zestaw konwersji DPI?',
      answer: 'Zestaw konwersji DPI zawiera głowicę drukującą o docelowej rozdzielczości oraz elementy potrzebne do zmiany gęstości druku w danej drukarce. Dzięki temu zmienisz rozdzielczość (np. z 203 na 300 DPI) bez kupowania nowej maszyny — wymieniasz tylko głowicę i powiązane części, a następnie aktualizujesz ustawienia drukarki.'
    },
    {
      question: 'Czy zmiana DPI wymaga serwisu?',
      answer: 'Montaż obejmuje wymianę głowicy i zmianę ustawień rozdzielczości, co wymaga ostrożności i kalibracji. Doświadczony dział utrzymania ruchu zwykle radzi sobie sam, ale zalecamy montaż w autoryzowanym serwisie Zebra — gwarantuje to poprawną kalibrację i zachowanie gwarancji.'
    },
    {
      question: 'Czym różni się 203 DPI od 300 DPI?',
      answer: '203 DPI (8 punktów/mm) to standard do etykiet logistycznych i typowych kodów kreskowych — szybciej i taniej. 300 DPI (12 punktów/mm) daje wyższą jakość przy drobnych kodach 2D, małym tekście i etykietach farmaceutycznych. Wybierz rozdzielczość według najmniejszego elementu, jaki musisz wydrukować czytelnie.'
    },
    {
...
export async function generateMetadata({ params }: { params: { slug: string[] } }): Promise<Metadata> {
  ...
  if (slugPath.length === 1) {
    ...
    // Akumulatory: cena "od X zł" z realnego minimum DOSTĘPNEGO produktu
    if (productType.slug === 'akumulatory') {
      const akumulatory = await getProductsForCategory({ productType: 'akumulator' })
      ...
      return {
        ...
        openGraph: {
          ...
          locale: 'pl_PL',
          images: [{
            url: 'https://www.serwis-zebry.pl/sklep_photo/BTRY-MC93-STN-01.png',
            width: 800,
            height: 800,
            alt: 'Oryginalny akumulator do terminala Zebra MC9400'
          }]
        },
        alternates: {
  ...
  if (slugPath.length === 2 && printerCategory) {
    ...
    if (productType.id === 'glowica' && printerCategory.id === 'desktop') {
      return {
        title: 'Głowice do drukarek biurkowych Zebra ZD421, GK420 | TAKMA',
    ...
    // SEO dla akumulatorów do terminali
    ...
    // SEO dla akumulatorów do drukarek mobilnych
    if (productType.id === 'akumulator' && printerCategory.id === 'mobile') {
      return {
        ...
        openGraph: {
          ...
          locale: 'pl_PL',
          images: [{
            url: 'https://www.serwis-zebry.pl/sklep_photo/BTRY-MPP-34MA1-01.png',
            width: 800,
            height: 800,
            alt: 'Oryginalny akumulator do drukarki mobilnej Zebra ZQ511'
          }]
        },
        alternates: {
...
export default async function ShopCategoryPage({ params }: { params: { slug: string[] } }) {
  ...
  const collectionPagesMap: Record<string, { url: string; description: string }> = {
    ...
    'glowice/drukarki-biurkowe': {
      url: 'https://www.serwis-zebry.pl/sklep/glowice/drukarki-biurkowe',
      description: 'Oryginalne głowice do drukarek biurkowych Zebra serii ZD, GK i GX w rozdzielczościach 203 i 300 DPI.'
    },
    ...
    'akumulatory/terminale': {
      url: 'https://www.serwis-zebry.pl/sklep/akumulatory/terminale',
      description: 'Oryginalne akumulatory do terminali Zebra serii TC i MC — warianty Standard, Extended i Freezer.'
    },
    'akumulatory/drukarki-mobilne': {
      url: 'https://www.serwis-zebry.pl/sklep/akumulatory/drukarki-mobilne',
      description: 'Oryginalne akumulatory do drukarek mobilnych Zebra serii ZQ — od ZQ220 Plus do ZQ630 Plus.'
    },
    'konwertery': {
      url: 'https://www.serwis-zebry.pl/sklep/konwertery',
      description: 'Oryginalne zestawy konwersji rozdzielczości DPI do drukarek przemysłowych Zebra — zmiana 203, 300 i 600 DPI w seriach ZT i print engine ZE bez wymiany drukarki.'
    },
    'konwertery/drukarki-przemyslowe': {
      url: 'https://www.serwis-zebry.pl/sklep/konwertery/drukarki-przemyslowe',
      description: 'Zestawy konwersji DPI do drukarek przemysłowych Zebra serii ZT111/211/231, ZT410/411/420/421 oraz ZT210/220/230 — zmiana rozdzielczości głowicy 203↔300↔600 DPI.'
    }
  }
  const glowiceCollection = (productType.id === 'glowica' || productType.id === 'walek' || productType.id === 'akumulator' || productType.id === 'zasilacz' || productType.id === 'konwerter')
  ...
  const collectionPageSchema = glowiceCollection ? {
    ...
    "@type": "CollectionPage",
    "name": pageTitle,
    "description": glowiceCollection.description,
    "url": glowiceCollection.url,
    "isPartOf": {
      "@type": "WebSite",
      "name": "TAKMA - Autoryzowany Serwis Zebra",
  ...
  const walekMaxPrice = productType.id === 'walek' && products.length > 0
    ? Math.ceil(Math.max(...products.map(p => p.price)))
    : 536
  // Analogicznie dla akumulatorów
  const akumAvailablePrices = productType.id === 'akumulator'
    ? products.filter(p => (p.stock ?? 0) > 0).map(p => p.price)
    : []
  const akumMinPrice = akumAvailablePrices.length > 0 ? Math.floor(Math.min(...akumAvailablePrices)) : 205
  ...
  const zasilaczMaxPrice = productType.id === 'zasilacz' && products.length > 0
    ? Math.ceil(Math.max(...products.map(p => p.price)))
    : 2086
  // Analogicznie dla konwerterów DPI
  const konwerterAvailablePrices = productType.id === 'konwerter'
    ? products.filter(p => (p.stock ?? 0) > 0).map(p => p.price)
    : []
  const konwerterMinPrice = konwerterAvailablePrices.length > 0 ? Math.floor(Math.min(...konwerterAvailablePrices)) : 1398
  const konwerterMaxPrice = productType.id === 'konwerter' && products.length > 0
    ? Math.ceil(Math.max(...products.map(p => p.price)))
    : 5682
  ...
  const BATTERY_SPECS: Record<string, string> = {
    'BTRY-TC2Y-1XMA1-01': 'Standard 3100 mAh',
    'BTRY-TC2Y-2XMA1-01': 'Extended 5200 mAh',
    'BTRY-TC2L-2XMAXX-01': 'Standard 3800 mAh',
    'BTRY-TC2L-3XMAXX-01': 'Extended 5200 mAh',
    'BTRY-NGTC5TC7-44MA-01': 'Standard 4400 mAh',
    'BTRY-NGTC5TC7-66MA-01': 'Extended 6600 mAh',
    'BTRY-MC2X-35MA-01': 'Standard 3500 mAh',
    'BTRY-MC2X-49MA-01': 'Extended 4900 mAh',
    'BTRY-MC3X-70MA-01': 'Standard 7000 mAh',
    'BTRY-MC93-STN-01': 'Standard 5000 mAh',
    'BTRY-MC93-FRZ-01': 'Freezer 2900 mAh (-30°C)',
    'BTRY-MPV-25MAC1-01': 'Standard 2500 mAh',
    'BTRY-MPM-22MA1-01': 'Standard 2200 mAh',
    'BTRY-MPP-34MA1-01': 'Standard 3400 mAh',
    'BTRY-MPP-EXT1-01': 'Extended 6800 mAh',
    'BTRY-MPP-68MA1-01': 'Standard 6800 mAh',
  }
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
        ...
        {/* SEO Content Section - Konwertery DPI */}
        {productType.id === 'konwerter' && slugPath.length === 1 && (
          <>
          <section className="py-8 sm:py-12 bg-white border-t border-gray-100">
            <div className="max-w-4xl mx-auto px-4">
              <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-6">
                Konwertery DPI do drukarek Zebra — zmiana rozdzielczości bez wymiany drukarki
              </h2>

              <div className="prose prose-sm sm:prose-base prose-gray max-w-none">
                <p className="text-gray-600 leading-relaxed mb-4">
                  <strong>Zestaw konwersji DPI</strong> pozwala zmienić rozdzielczość drukowania drukarki
                  przemysłowej Zebra — najczęściej <strong>konwersja 203 na 300 DPI</strong> albo z powrotem
                  z 300 na 203 DPI — bez kupowania nowej maszyny. To realna oszczędność, gdy zmieniają się
                  wymagania: drobne kody 2D, etykiety farmaceutyczne czy mały tekst potrzebują 300 DPI,
                  a szybki druk etykiet logistycznych wystarczy w 203 DPI. Zmiana rozdzielczości drukarki
                  Zebra polega na wymianie głowicy na docelową gęstość punktów wraz z elementami zestawu
                  i aktualizacją ustawień. Ceny zestawów zaczynają się od {formatPln(konwerterMinPrice)} zł netto.
                </p>

                <h3 className="text-lg font-semibold text-gray-900 mt-6 mb-3">
                  203 czy 300 DPI — co wybrać?
                </h3>
                <p className="text-gray-600 leading-relaxed mb-4">
                  <strong>203 DPI</strong> (8 punktów/mm) to standard do etykiet logistycznych, wysyłkowych
                  i typowych kodów kreskowych — drukuje szybciej i taniej. <strong>300 DPI</strong>
                  (12 punktów/mm) daje wyraźnie wyższą jakość: drobne kody 2D (DataMatrix, QR), mały tekst,
                  etykiety farmaceutyczne i jubilerskie. Niektóre modele (ZT411, ZT610) obsługują też
                  <strong> 600 DPI</strong> do najdrobniejszych nadruków. Jeśli kupujesz drukarkę „na zapas",
                  konwersja w drugą stronę zawsze jest możliwa odpowiednim zestawem.
                </p>

                <h3 className="text-lg font-semibold text-gray-900 mt-6 mb-3">
                  Tabela Part Numbers — konwertery DPI Zebra
                </h3>
                <div className="overflow-x-auto mb-6">
                  {/* Tabela generowana z bazy produktów — PN, ceny i dostępność zawsze aktualne */}
                  <table className="w-full text-sm">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mt-6 mb-3">
                  Dla jakich drukarek Zebra mamy zestawy konwersji?
                </h3>
                <ul className="text-gray-600 space-y-2 mb-4">
                  <li><strong>Seria ZT111 / ZT211 / ZT231</strong> — kompaktowe przemysłowe, konwersja 203 ↔ 300 DPI.</li>
                  <li><strong>Seria ZT410 / ZT411 / ZT420 / ZT421</strong> — najpopularniejsze przemysłowe; konwersja 203 ↔ 300 DPI, a dla ZT411 także do 600 DPI.</li>
                  <li><strong>Seria ZT210 / ZT220 / ZT230</strong> — starsze przemysłowe, konwersja 203 ↔ 300 DPI.</li>
                  <li><strong>Print engine ZE511 / ZE521</strong> — moduły druku do linii produkcyjnych, konwersja 203 ↔ 300 DPI.</li>
                </ul>
                ...
                <div className="space-y-4 mb-6">
                  <div className="border border-slate-200 rounded-xl p-4">
                    <p className="font-semibold text-gray-900">Czy mogę zmienić rozdzielczość drukarki Zebra z 203 na 300 DPI?</p>
                    <p className="text-gray-600 text-sm mt-1">Tak. Zmiana rozdzielczości drukarki Zebra polega na zamontowaniu zestawu konwersji DPI z głowicą docelowej gęstości (np. konwersja 203 na 300 DPI w ZT411) i aktualizacji ustawień. Sama drukarka pozostaje ta sama — wymieniasz tylko głowicę i powiązane elementy.</p>
                  </div>
                  <div className="border border-slate-200 rounded-xl p-4">
                    <p className="font-semibold text-gray-900">Czym różni się 203 DPI od 300 DPI?</p>
                    <p className="text-gray-600 text-sm mt-1">203 DPI (8 pkt/mm) to standard logistyczny — szybciej i taniej. 300 DPI (12 pkt/mm) daje wyższą jakość przy drobnych kodach 2D, małym tekście i etykietach farmaceutycznych. Wybór zależy od najmniejszego elementu, jaki musisz wydrukować czytelnie.</p>
                  </div>
                  <div className="border border-slate-200 rounded-xl p-4">
                    <p className="font-semibold text-gray-900">Ile kosztuje zmiana DPI w drukarce Zebra?</p>
                    <p className="text-gray-600 text-sm mt-1">Zestawy konwersji DPI kosztują od {formatPln(konwerterMinPrice)} zł do {formatPln(konwerterMaxPrice)} zł netto, zależnie od modelu i kierunku konwersji. To znacznie taniej niż zakup nowej drukarki o innej rozdzielczości.</p>
                  </div>
                  <div className="border border-slate-200 rounded-xl p-4">
                    <p className="font-semibold text-gray-900">Czy konwersję DPI mogę zrobić samodzielnie?</p>
                    <p className="text-gray-600 text-sm mt-1">Montaż zestawu obejmuje wymianę głowicy i zmianę ustawień rozdzielczości — wymaga ostrożności i kalibracji. Dział utrzymania ruchu zwykle radzi sobie sam, ale zalecamy montaż w autoryzowanym serwisie Zebra, żeby mieć pewność kalibracji i zachować gwarancję.</p>
                  </div>
                  <div className="border border-slate-200 rounded-xl p-4">
                    <p className="font-semibold text-gray-900">Czy do każdej drukarki Zebra jest zestaw konwersji?</p>
                    <p className="text-gray-600 text-sm mt-1">Zestawy konwersji DPI są dostępne dla drukarek przemysłowych serii ZT (ZT111/211/231, ZT410/411/420/421, ZT210/220/230) oraz print engine ZE511/ZE521. Konwersja do 600 DPI dotyczy modeli, które tę rozdzielczość obsługują (m.in. ZT411).</p>
                  </div>
                </div>

                <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 mt-6">
                  <p className="text-blue-800 text-sm">
                    <strong>Nie wiesz, który zestaw pasuje?</strong> Podaj model drukarki i docelową rozdzielczość —
                    dobierzemy właściwy zestaw konwersji DPI i, jeśli chcesz, wykonamy montaż z kalibracją w serwisie.
          ...
          {/* FAQPage Schema — /sklep/konwertery */}
          <script
            type="application/ld+json"
            dangerouslySetInnerHTML={{ __html: JSON.stringify({
              "@context": "https://schema.org",
              "@type": "FAQPage",
              "mainEntity": [
                { "@type": "Question", "name": "Czy mogę zmienić rozdzielczość drukarki Zebra z 203 na 300 DPI?", "acceptedAnswer": { "@type": "Answer", "text": "Tak. Zmiana rozdzielczości polega na zamontowaniu zestawu konwersji DPI z głowicą docelowej gęstości (np. konwersja 203 na 300 DPI w ZT411) i aktualizacji ustawień. Drukarka pozostaje ta sama." }},
                { "@type": "Question", "name": "Czym różni się 203 DPI od 300 DPI?", "acceptedAnswer": { "@type": "Answer", "text": "203 DPI (8 pkt/mm) to standard logistyczny — szybciej i taniej. 300 DPI (12 pkt/mm) daje wyższą jakość przy drobnych kodach 2D, małym tekście i etykietach farmaceutycznych." }},
          ...
          <section className="py-8 sm:py-12 bg-white border-t border-gray-100">
            <div className="max-w-4xl mx-auto px-4">
              ...
              <div className="prose prose-sm sm:prose-base prose-gray max-w-none">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mt-6 mb-3">
                  Tabela Part Numbers — akumulatory Zebra
                </h3>
                <div className="overflow-x-auto mb-6">
                  {/* Tabela generowana z bazy produktów — PN, ceny i dostępność zawsze aktualne */}
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-3 py-2 text-left font-semibold">Urządzenie</th>
                        <th className="px-3 py-2 text-left font-semibold">Typ / pojemność</th>
                        <th className="px-3 py-2 text-left font-semibold">Part Number</th>
                        <th className="px-3 py-2 text-left font-semibold">Cena (netto)</th>
                        <th className="px-3 py-2 text-left font-semibold">Dostępność</th>
                      </tr>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {['Terminale mobilne', 'Drukarki mobilne', 'Tablety'].map(group => {
                        const rows = [...products]
                          .filter(p => p.device_model && batteryGroup(p.device_model) === group)
                          .sort((a, b) => a.device_model.localeCompare(b.device_model, 'pl') || a.sku.localeCompare(b.sku))
                        if (rows.length === 0) return null
                        return (
                          <Fragment key={group}>
                            <tr className="bg-gray-50/50"><td colSpan={5} className="px-3 py-1.5 font-semibold text-xs text-gray-500 uppercase tracking-wider">{group}</td></tr>
                            {rows.map(p => (
                              <tr key={p.sku}>
                                <td className="px-3 py-2 font-medium">{p.device_model}</td>
                                <td className="px-3 py-2">{BATTERY_SPECS[p.sku] ?? '—'}</td>
                                <td className="px-3 py-2 font-mono text-xs">{p.sku}</td>
                                <td className="px-3 py-2">{formatPln(Math.round(p.price))} zł</td>
                                <td className="px-3 py-2">{(p.stock ?? 0) > 0 ? 'Dostępny' : 'Chwilowo niedostępny'}</td>
                              </tr>
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div className="border border-slate-200 rounded-xl p-4">
                    <p className="font-semibold text-gray-900">Ile kosztuje akumulator do urządzenia Zebra?</p>
                    <p className="text-gray-600 text-sm mt-1">Oryginalne akumulatory Zebra kosztują od {formatPln(akumMinPrice)} zł do {formatPln(akumMaxPrice)} zł netto — najtańsze są baterie Standard do terminali TC, najdroższe wysokopojemne baterie do tabletów ET i wariant Freezer do MC9400. Aktualne ceny i dostępność znajdziesz w tabeli powyżej.</p>
                  </div>
                  <div className="border border-slate-200 rounded-xl p-4">
          ...
          <script
            ...
            dangerouslySetInnerHTML={{ __html: JSON.stringify({
              ...
              "mainEntity": [
                ...
                { "@type": "Question", "name": "Ile kosztuje akumulator do urządzenia Zebra?", "acceptedAnswer": { "@type": "Answer", "text": `Oryginalne akumulatory Zebra kosztują od ${formatPln(akumMinPrice)} zł do ${formatPln(akumMaxPrice)} zł netto — najtańsze są baterie Standard do terminali TC, najdroższe baterie do tabletów ET i wariant Freezer.` }},
          ...
          <section className="py-8 sm:py-12 bg-white border-t border-gray-100">
            <div className="max-w-4xl mx-auto px-4">
              ...
              <div className="prose prose-sm sm:prose-base prose-gray max-w-none">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mt-6 mb-3">
                  Tabela Part Numbers — baterie do terminali Zebra
                </h3>
                <div className="overflow-x-auto mb-6">
                  {/* Tabela generowana z bazy produktów — PN, ceny i dostępność zawsze aktualne */}
                  <table className="w-full text-sm">
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div className="border border-slate-200 rounded-xl p-4">
                    ...
                    <p className="text-gray-600 text-sm mt-1">Oryginalne baterie do terminali Zebra kosztują od {formatPln(akumMinPrice)} zł do {formatPln(akumMaxPrice)} zł netto w zależności od modelu i wariantu (Standard/Extended/Freezer). Przy żywotności 2-3 lata i 250 dniach pracy rocznie koszt baterii to mniej niż 1 zł dziennie — wielokrotnie mniej niż strata produktywności z powodu rozładowanego terminala.</p>
                  </div>
                  <div className="border border-slate-200 rounded-xl p-4">
          ...
          <script
            ...
            dangerouslySetInnerHTML={{ __html: JSON.stringify({
              ...
              "mainEntity": [
                ...
                { "@type": "Question", "name": "Jaki jest koszt baterii do terminala Zebra?", "acceptedAnswer": { "@type": "Answer", "text": `Oryginalne baterie do terminali Zebra kosztują od ${formatPln(akumMinPrice)} zł do ${formatPln(akumMaxPrice)} zł netto w zależności od modelu i wariantu (Standard/Extended/Freezer).` }},
          ...
          <section className="py-8 sm:py-12 bg-white border-t border-gray-100">
            <div className="max-w-4xl mx-auto px-4">
              ...
              <div className="prose prose-sm sm:prose-base prose-gray max-w-none">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mt-6 mb-3">
                  Tabela Part Numbers — baterie do drukarek mobilnych
                </h3>
                <div className="overflow-x-auto mb-6">
                  {/* Tabela generowana z bazy produktów — PN, ceny i dostępność zawsze aktualne */}
                  <table className="w-full text-sm">
        ...
        {productType.id === 'glowica' && slugPath.length === 2 && printerCategory?.id === 'industrial' && (
          <section className="py-8 sm:py-12 bg-white border-t border-gray-100">
            <div className="max-w-4xl mx-auto px-4">
              ...
              <div className="prose prose-sm sm:prose-base prose-gray max-w-none">
                <p className="text-gray-600 leading-relaxed mb-4">
                  <strong>Drukarki przemysłowe Zebra</strong> (seria ZT, 105SL, ZM) to urządzenia zaprojektowane do
                  ...
                  i drukowaniu milionów etykiet rocznie. Najczęściej wymieniane modele to głowica ZT411
                  (następca ZT410) oraz głowica ZT610 — oba dostępne w trzech rozdzielczościach.
                  Ceny głowic przemysłowych zaczynają się od {formatPln(glowicaMinPrice)} zł netto.
                </p>

                <h3 className="text-lg font-semibold text-gray-900 mt-6 mb-3">
                  Szerokość druku w drukarkach przemysłowych
                </h3>
                <p className="text-gray-600 leading-relaxed mb-4">
                  Przed zakupem zwróć uwagę na szerokość druku — głowice do drukarek 104 mm (4") i 168 mm (6.6")
                  nie są wymienne. ZT411, ZT510, ZT610, 105SL Plus i ZM400 drukują na szerokości <strong>104 mm</strong>,
                  natomiast ZT421 i ZT620 to maszyny <strong>168 mm</strong> — ich głowice są fizycznie dłuższe
                  i wyraźnie droższe. Jeśli masz wątpliwości, sprawdź symbol modelu na tabliczce z przodu drukarki.
                </p>

                <h3 className="text-lg font-semibold text-gray-900 mt-6 mb-3">
                  Tabela Part Numbers — głowice przemysłowe
                </h3>
                <div className="overflow-x-auto mb-6">
                  {/* Tabela generowana z bazy produktów — ceny i dostępność zawsze aktualne */}
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-3 py-2 text-left font-semibold">Model drukarki</th>
                ...
                <p className="text-gray-600 leading-relaxed mb-4">
                  Pamiętaj, że rozdzielczość głowicy musi zgadzać się z rozdzielczością, w jakiej drukarka
                  została skonfigurowana. Montaż głowicy 300 DPI w drukarce ustawionej na 203 DPI skończy się
                  zniekształconym wydrukiem — po zmianie rozdzielczości konieczna jest rekalibracja
                  i korekta szablonów etykiet (wymiary w punktach zmieniają się o 50%).
                </p>

                <h3 className="text-lg font-semibold text-gray-900 mt-6 mb-3">
                  Żywotność głowic przy pracy ciągłej
                </h3>
                <p className="text-gray-600 leading-relaxed mb-4">
                  ...
                  więc zużywa się szybciej niż 203 DPI. W drukarkach pracujących na trzy zmiany największym
                  wrogiem głowicy jest zabrudzony wałek i tani ribbon bez warstwy ochronnej — regularne
                  czyszczenie alkoholem IPA 99% przy każdej zmianie rolki wydłuża żywotność nawet o 50%.
                  Warto też kontrolować ustawienie zaczernienia: wartości powyżej 25 przyspieszają wypalanie
                  elementów grzewczych, a zwykle wystarczy poprawić docisk lub wymienić ribbon na lepszy.
                </p>

                <h3 className="text-lg font-semibold text-gray-900 mt-6 mb-3">
                  Wymiana głowicy w drukarce przemysłowej Zebra
                </h3>
                <p className="text-gray-600 leading-relaxed mb-4">
                  W serii ZT wymiana głowicy zajmuje kilkanaście minut: wyłącz drukarkę, otwórz pokrywę,
                  zwolnij dźwignię docisku, odłącz taśmy przewodów i odkręć śrubę mocującą. Po montażu nowej
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div className="border border-slate-200 rounded-xl p-4">
                    <p className="font-semibold text-gray-900">Czy głowica ZT610 pasuje do ZT620?</p>
                    <p className="text-gray-600 text-sm mt-1">Nie. ZT610 drukuje na szerokości 104 mm, a ZT620 na 168 mm —
                    głowice mają różną długość i osobne Part Numbers (ZT610: P1083320-010/-011/-012, ZT620: P1083320-015/-016).</p>
                  </div>
                  <div className="border border-slate-200 rounded-xl p-4">
                  ...
                  <div className="border border-slate-200 rounded-xl p-4">
                    ...
                    <p className="text-gray-600 text-sm mt-1">Oryginalne głowice do drukarek przemysłowych Zebra kosztują
                    od {formatPln(glowicaMinPrice)} zł do {formatPln(glowicaMaxPrice)} zł netto w zależności od modelu,
                    szerokości druku i rozdzielczości. Najdroższe są głowice 600 DPI i głowice do maszyn 168 mm.</p>
                  </div>
                  <div className="border border-slate-200 rounded-xl p-4">
                    <p className="font-semibold text-gray-900">Po wymianie głowicy wydruk jest blady po jednej stronie — co zrobić?</p>
                    <p className="text-gray-600 text-sm mt-1">To niemal zawsze nierówny docisk głowicy. Wyreguluj pokrętła
                    docisku nad głowicą (w ZT411/ZT610 osobne dla lewej i prawej strony) i wykonaj wydruk testowy.
                    Jeśli to nie pomoże, sprawdź czy wałek dociskowy nie jest zużyty.</p>
                  </div>
                </div>
          ...
          <script
            ...
            dangerouslySetInnerHTML={{ __html: JSON.stringify({
              ...
              "mainEntity": [
                ...
                { "@type": "Question", "name": "Czy głowica ZT610 pasuje do ZT620?", "acceptedAnswer": { "@type": "Answer", "text": "Nie. ZT610 drukuje na szerokości 104 mm, a ZT620 na 168 mm — głowice mają różną długość i osobne Part Numbers (ZT610: P1083320-010/-011/-012, ZT620: P1083320-015/-016)." }},
                ...
                { "@type": "Question", "name": "Ile kosztuje głowica do drukarki przemysłowej?", "acceptedAnswer": { "@type": "Answer", "text": `Oryginalne głowice do drukarek przemysłowych Zebra kosztują od ${formatPln(glowicaMinPrice)} zł do ${formatPln(glowicaMaxPrice)} zł netto w zależności od modelu, szerokości druku i rozdzielczości. Najdroższe są głowice 600 DPI i głowice do maszyn 168 mm.` }},
                { "@type": "Question", "name": "Po wymianie głowicy wydruk jest blady po jednej stronie — co zrobić?", "acceptedAnswer": { "@type": "Answer", "text": "To niemal zawsze nierówny docisk głowicy. Wyreguluj pokrętła docisku nad głowicą (w ZT411/ZT610 osobne dla lewej i prawej strony) i wykonaj wydruk testowy. Jeśli to nie pomoże, sprawdź czy wałek dociskowy nie jest zużyty." }}
              ]
            }) }}
          />
        )}
            ...
            <div className="max-w-4xl mx-auto px-4">
              ...
              <div className="prose prose-sm sm:prose-base prose-gray max-w-none">
                <p className="text-gray-600 leading-relaxed mb-4">
                  ...
                  równie starannego doboru. Najczęściej kupowana jest głowica ZD421 (osobne wersje
                  dla ZD421d i ZD421t) oraz głowica GK420 do wciąż licznych na rynku drukarek GK420d i GK420t.
                  Ceny zaczynają się od {formatPln(glowicaMinPrice)} zł netto.
                </p>

                <h3 className="text-lg font-semibold text-gray-900 mt-6 mb-3">
                  Tabela Part Numbers — głowice biurkowe
                </h3>
                <div className="overflow-x-auto mb-6">
                  {/* Tabela generowana z bazy produktów — ceny i dostępność zawsze aktualne */}
                  <table className="w-full text-sm">
                ...
                <p className="text-gray-600 text-sm mb-4">
                  <strong>Uwaga:</strong> wersja termiczna (d) i termotransferowa (t) tego samego modelu ma
                  <strong> osobne Part Numbers</strong> — np. głowica do ZD421d to P1112640-019, a do ZD421t
                  P1112640-218. Przy zamawianiu zawsze sprawdzaj pełny symbol modelu z literą na końcu.
                </p>

                <h3 className="text-lg font-semibold text-gray-900 mt-6 mb-3">
                  Wymiana głowicy w serii ZD — bez narzędzi
                </h3>
                <p className="text-gray-600 leading-relaxed mb-4">
                  W drukarkach ZD411, ZD421, ZD611 i ZD621 głowicę wymienisz bez śrubokręta: wyłącz drukarkę,
                  otwórz pokrywę, zwolnij zatrzask mocujący głowicę, odepnij dwa przewody i zamontuj nową
                  w odwrotnej kolejności. Cała operacja zajmuje ok. 5 minut. W starszych GK420 i GX420
                  potrzebny jest krzyżakowy śrubokręt (dwie śruby). Po każdej wymianie wykonaj
                  kalibrację czujników — bez niej drukarka może źle pozycjonować etykiety.
                </p>

                <h3 className="text-lg font-semibold text-gray-900 mt-6 mb-3">
                  Wymiana głowicy czy nowa drukarka?
                </h3>
                <p className="text-gray-600 leading-relaxed mb-4">
                  Przy drukarkach biurkowych warto policzyć opłacalność: głowica 203 DPI do ZD421 kosztuje
                  niecałe 500 zł netto, do GK420 — ok. 460-490 zł, podczas gdy nowa
                  drukarka to wydatek 1200-2000 zł. Jeśli mechanika drukarki jest sprawna, wymiana głowicy
                  zwraca się zawsze. Wyjątek to wyeksploatowane GK420 z uszkodzonym wałkiem i przekładniami —
                  wtedy rozsądniej zainwestować w nowszą ZD421, która drukuje szybciej i ma łączność USB-C
                  lub opcjonalnie Wi-Fi/Bluetooth.
                </p>
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div className="border border-slate-200 rounded-xl p-4">
                    <p className="font-semibold text-gray-900">Czy głowica do ZD421d pasuje do ZD421t?</p>
                    <p className="text-gray-600 text-sm mt-1">Nie — mimo identycznej obudowy drukarki wersje d i t mają
                    osobne głowice: P1112640-019/-020 (ZD421d) oraz P1112640-218/-219 (ZD421t).
                    Dobieraj część po pełnym symbolu modelu.</p>
                  </div>
                  <div className="border border-slate-200 rounded-xl p-4">
                    ...
                    od {formatPln(glowicaMinPrice)} zł do {formatPln(glowicaMaxPrice)} zł netto.
                    Głowice 300 DPI są ok. dwukrotnie droższe od 203 DPI.</p>
                  </div>
                  <div className="border border-slate-200 rounded-xl p-4">
                    <p className="font-semibold text-gray-900">Po czym poznać zużytą głowicę w drukarce biurkowej?</p>
                    <p className="text-gray-600 text-sm mt-1">Pionowe białe linie na wydruku, blade fragmenty etykiety
                    mimo zwiększonego zaczernienia i nieczytelne kody kreskowe. Jeśli linia jest zawsze w tym samym
                    miejscu — element grzewczy jest przepalony i czyszczenie już nie pomoże.</p>
                  </div>
                  <div className="border border-slate-200 rounded-xl p-4">
                    <p className="font-semibold text-gray-900">Czy mogę wymienić głowicę samodzielnie bez utraty gwarancji?</p>
                    <p className="text-gray-600 text-sm mt-1">Tak — głowica jest częścią eksploatacyjną, a jej wymiana
                    w serii ZD nie wymaga narzędzi i nie narusza plomb. Na samą głowicę otrzymujesz 12 miesięcy
          ...
          <script
            ...
            dangerouslySetInnerHTML={{ __html: JSON.stringify({
              ...
              "mainEntity": [
                ...
                { "@type": "Question", "name": "Czy głowica do ZD421d pasuje do ZD421t?", "acceptedAnswer": { "@type": "Answer", "text": "Nie — mimo identycznej obudowy drukarki wersje d i t mają osobne głowice: P1112640-019/-020 (ZD421d) oraz P1112640-218/-219 (ZD421t). Dobieraj część po pełnym symbolu modelu." }},
                { "@type": "Question", "name": "Jaka jest cena głowicy do drukarki biurkowej?", "acceptedAnswer": { "@type": "Answer", "text": `Oryginalne głowice Zebra do drukarek biurkowych kosztują od ${formatPln(glowicaMinPrice)} zł do ${formatPln(glowicaMaxPrice)} zł netto. Głowice 300 DPI są ok. dwukrotnie droższe od 203 DPI.` }},
                { "@type": "Question", "name": "Po czym poznać zużytą głowicę w drukarce biurkowej?", "acceptedAnswer": { "@type": "Answer", "text": "Pionowe białe linie na wydruku, blade fragmenty etykiety mimo zwiększonego zaczernienia i nieczytelne kody kreskowe. Jeśli linia jest zawsze w tym samym miejscu — element grzewczy jest przepalony i czyszczenie już nie pomoże." }},
                { "@type": "Question", "name": "Czy mogę wymienić głowicę samodzielnie bez utraty gwarancji?", "acceptedAnswer": { "@type": "Answer", "text": "Tak — głowica jest częścią eksploatacyjną, a jej wymiana w serii ZD nie wymaga narzędzi i nie narusza plomb. Na samą głowicę otrzymujesz 12 miesięcy gwarancji producenta." }}
              ]
            }) }}
          />
        )}
