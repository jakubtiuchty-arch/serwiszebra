        ...
        <div className="border-b border-gray-200">
          <div className="flex items-center justify-between px-4 py-3">
            <div className="flex gap-0.5 bg-gray-100 rounded-lg p-0.5">
              ...
              >
                W trakcie
                <span className={`ml-1.5 px-1.5 py-0.5 rounded-full text-[10px] font-semibold ${filter === 'w_trakcie' ? 'bg-white/20 text-white' : 'bg-gray-200 text-gray-700'}`}>{stats.inProgress}</span>
              </button>
              <button
              ...
              >
                Dostarczone
                <span className={`ml-1.5 px-1.5 py-0.5 rounded-full text-[10px] font-semibold ${filter === 'dostarczone' ? 'bg-white/20 text-white' : 'bg-gray-200 text-gray-700'}`}>{stats.delivered}</span>
              </button>
            </div>
          </div>
        </div>
              ...
              {filteredOrders.map((order) => {
                ...
                return (
                  <div key={order.id} className="relative">
                    <div className="bg-white border-2 border-gray-300 rounded-xl shadow-md hover:shadow-lg hover:border-gray-400 transition-all group relative overflow-hidden">
                      ...
                      {canDeleteOrder(order.status) && (
                        <button
                          ...
                          className="absolute top-3 right-3 w-6 h-6 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 hover:scale-110 z-10 shadow-lg"
                      ...
                      <div className="relative flex gap-3 p-3">
                        <div className="hidden sm:flex flex-col items-center gap-2 flex-shrink-0">
                          <div className={`relative w-10 h-10 ${statusConfig.bg} rounded-full flex items-center justify-center ring-2 ring-white group-hover:scale-110 transition-transform`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="sm:hidden flex items-center gap-2 mb-2">
                            <div className={`relative w-8 h-8 ${statusConfig.bg} rounded-full flex items-center justify-center`}>
                              <StatusIcon className={`w-4 h-4 ${statusConfig.text}`} />
                            </div>
                            <span className={`px-2 py-0.5 ${statusConfig.bg} ${statusConfig.text} rounded-md text-[10px] font-bold uppercase tracking-wide`}>
                              {statusConfig.label}
                            </span>
                          </div>

                          <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2 mb-2">
                            <span className={`hidden sm:inline-flex px-2 py-1 ${statusConfig.bg} ${statusConfig.text} rounded-md text-[10px] font-bold uppercase tracking-wide`}>
                              {statusConfig.label}
                            </span>
                          ...
                          {order.status !== 'cancelled' && (
                            <div className="mb-3">
                              <div className="flex items-center justify-between mb-1">
                                <span className="text-[10px] font-semibold text-gray-600">Status realizacji</span>
                                <span className="text-[10px] font-bold text-gray-900">{statusConfig.progress}%</span>
                              </div>
                          ...
                          <button
                            ...
                            className="flex items-center gap-1.5 text-[10px] text-gray-600 hover:text-gray-900 mb-3 transition-colors"
                          >
                          ...
                          {isExpanded && order.items && (
                            <div className="mb-3 bg-gray-50 rounded-lg p-2.5 space-y-1.5">
                              {order.items.map((item) => (
                                <div key={item.id} className="flex items-center justify-between text-[10px] gap-2">
                                  <div className="flex-1 min-w-0">
                              ...
                              <div className="pt-1.5 mt-1.5 border-t border-gray-200">
                                <div className="flex items-center justify-between text-[10px]">
                                  <p className="font-semibold text-gray-900">Razem brutto:</p>
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 pt-2 border-t border-gray-200">
                            <div className="flex flex-wrap items-center gap-1.5">
                              <div className={`flex items-center gap-1 ${paymentConfig.color} font-semibold text-[10px]`}>
                              ...
                              {order.paymentStatus === 'paid' && order.invoiceNumber && (
                                <button
                                  ...
                                  className="flex items-center gap-1 px-2 py-1 bg-blue-100 hover:bg-blue-200 text-blue-700 rounded-md text-[10px] font-medium transition-all"
                                >
                              ...
                              {(order.status === 'shipped' || order.status === 'delivered') && order.tracking_number && (
                                <button
                                  ...
                                  className="flex items-center gap-1 px-2 py-1 bg-purple-100 hover:bg-purple-200 text-purple-700 rounded-md text-[10px] font-medium transition-all"
                                >
                              ...
                              {canReturnOrder(order.status) && (
                                <button
                                  ...
                                  className="flex items-center gap-1 px-2 py-1 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-md text-[10px] font-medium transition-all"
                                >
