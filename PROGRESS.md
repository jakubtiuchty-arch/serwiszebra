...
## 2026-08-31 — nowy hero sklepu z częściami (animowany)
- Poprzedni baner (komiksowa eksplozja z błyskawicami na granacie) odstawał od reszty sklepu; nowy trzyma styl kafelków klas: ciepłe światło z okna, halftone, limonkowe akcenty, spokojny blat serwisowy. Wygenerowany w Higgsfieldzie (gpt_image_2) z DWIEMA referencjami — kafelek klasy `/klasy/biurkowe.jpg` dla stylu i zdjęcie głowicy ze sklepu dla wierności części. Na blacie: głowica, wałek dociskowy, akumulator, zasilacz, rolka etykiet; lewe 45% kadru puste pod nagłówek.
...
- **Hero sklepu poprawiony po uwagach**: (1) animacja usunięta — `HeroWideo` i plik `.mp4` skasowane; (2) nowa grafika mówi „sklep", nie „warsztat": drukarka ZD421 (1:1 z renderu) w otwartym kartonie wysyłkowym, zaklejona paczka obok, rolki etykiet, tacka z częściami, taśma pakowa — czyli towar gotowy do wysyłki, bez narzędzi i rozłożonego sprzętu; (3) treść obejmuje CAŁY asortyment, nie tylko części — H1 „Urządzenia i części zamienne Zebra", akapit wymienia drukarki, terminale, skanery i tablety obok głowic, wałków i akumulatorów. Meta title/description `/sklep` przepisane w tym samym duchu (poprzednie mówiły wyłącznie o głowicach).
- **Hero sklepu przerobiony na kompozycję pasową** (uwaga użytkownika: obcinało górę i dół, a hero powiększyć nie wolno). Pomiar: pas ma 5,2:1 przy 1440 px i 6,9:1 przy 1920 px, więc grafika 16:9 traciła po ~35% wysokości z każdej strony. Nowa generacja z twardym wymogiem „wszystkie obiekty w środkowym pasie o wysokości 35% kadru, góra i dół puste", plik przycięty do 4:1 (2000 × 500 px, 259 kB) — teraz `object-cover` odcina tylko margines. Na telefonie `object-[62%_center]` przesuwa kadr tak, żeby było widać karton z drukarką, a nie same rolki przy krawędzi.
- Przy okazji: na telefonie tekst leżał na jasnej grafice (gradient po lewej działa tylko na szerokim ekranie) — tam przyciemnienie jest teraz równomierne, `bg-slate-950/75`.
- **Hero sklepu, druga podejście do kadru**: sam prompt („obiekty w środkowym pasie") nie wystarczał — model konsekwentnie stawiał scenę na dolnej połowie kadru 16:9, więc pas 5,2:1 wycięty ze środka i tak ucinał kartony. Rozwiązanie: `outpaint_image` rozszerzył wygenerowaną scenę do 21:9 (dorysowana ściana i blat po bokach), a pas wycięty z DOLNEJ części tej panoramy mieści cały rząd: zaklejony karton, otwarty karton z drukarką, rolki etykiet, tacka z akumulatorami i taśma pakowa. Plik 2400 × 464 px (5,18:1), 219 kB — dokładnie proporcja hero na 1440 px.

## 2026-08-31 — klasa mobilna: ZQ610 Plus, ZQ620 Plus, ZQ630 Plus
- Trzy karty serii ZQ600 Plus, po cztery warianty każda (Bluetooth+RS-232, Wi-Fi 5, Wi-Fi 6, Wi-Fi 6 linerless). **PN-ów nie dało się wyprowadzić z wzorca** — poprawny format to `ZQ6X-AU{F|W|X}AE14-00`, znaleziony w katalogu Jarltecha i zweryfikowany po kolei przez `product-stock`; wcześniejsze próby (`ZQ61-AUWA004-00`, `ZQ61-AUFA000-00`) nie istnieją u dystrybutorów.
- Dane od producenta (podręcznik ZQ600 Plus + spec sheet): druk 48 / 72 / 104 mm, 203 dpi, do 115 mm/s, nośniki 25,4–55,4 / 25,4–79,4 / 50,8–111 mm, wagi 0,6 / 0,73 / 1,11 kg, akumulatory 3250 / 3250 / 6600 mAh, IP54, praca −20…50°C, 512 MB Flash + 256 MB RAM, CPCL/ZPL/EPL. **Tylko ZQ630 Plus drukuje pełną etykietę kurierską** — to fakt otwierający „W skrócie" i FAQ każdej z trzech kart.
- **Filtr katalogu przebudowany na dane** (`zbudujGrupy`): kolumny powstają z cech, które faktycznie różnicują warianty w danej klasie. Biurkowe dostały przy okazji dwie nowe osie (Panel, Kolor), mobilne mają Łączność i Nośnik zamiast martwych chipów USB/Ethernet. Cecha o jednej wartości nie zajmuje miejsca — dlatego u mobilnych nie ma „Rozdzielczości" (wszystkie 203 dpi).
  - Klucze w adresie to slugi nazw cech (`?rozdzielczosc=300+dpi&lacznosc=Wi-Fi`), więc stare linki filtra przestały działać — akceptowalne, bo kombinacje i tak mają `noindex`.
  - Chipy na kafelku liczone z tych samych cech, więc mobilne pokazują łączność zamiast sztywnego „USB · LAN".
- `/sklep/drukarki-etykiet/mobilne` przestała być stroną „na zamówienie" — ma teraz katalog z filtrem, blok „w skrócie", opisy trzech modeli, tabelę porównawczą (szerokość, nośnik, waga, akumulator, etykieta kurierska), FAQ z FAQPage i ItemList.
- Skill `karta-produktu` przepisany na podstawie dziesięciu kart biurkowych: kolejność wdrożenia, weryfikacja PN, blok „W skrócie", data weryfikacji, poradniki po slugach, spójność cech z filtrem, BackOrder zamiast InStock dla towaru w drodze.

## 2026-09-01 — ZT411 i domknięcie klasy półprzemysłowej
- Karta **Zebra ZT411** (`d4b38dd`) — trzecia i ostatnia w klasie półprzemysłowej, po niej ZT111/ZT231/ZT411 stanowią komplet. Do tego przebudowana strona kategorii, most z instrukcji, sitemapy i llms.txt.
- Tabela wariantów ZT411 dostała krótkie nazwy wyposażenia (`d22e9a8`), przełącznik rozdzielczości części przeniesiony do nagłówka sekcji (`3ccbffb`).
- „Pasek" zastąpiony „szerokością wydruku" we wszystkich czterech kategoriach i w `lib/device-content.tsx` (`c93757e`) — klient szuka szerokości druku, nie żargonu.
- Zostaje pusta klasa przemysłowa: ZT421, ZT510, ZT610, ZT620 — cztery serie bez kart.

## 2026-09-01 — oferty promocyjne w panelu klienta (pierwsza: kontrakt serwisowy)
- **Decyzja o miejscu: pas w treści dashboardu, pod kaflami statystyk.** Nagłówek panelu odpada, bo to pasek narzędziowy z jedyną akcją klienta („Nowe zgłoszenie") — reklama obok niej konkuruje z tym, po co klient przyszedł. Sidebar odpada, bo ma 224 px szerokości i na telefonie jest schowany za hamburgerem, więc oferta nie dotarłaby do połowy odbiorców.
- **Pierwsza wersja (kafel z pełnym zakresem) odrzucona przez użytkownika**: za wysoka, ucinała grafikę i powtarzała treść landingu. Baner ma jedno zadanie — przenieść na `/kontrakt-serwisowy`. Na pasie zostały: eyebrow, nagłówek, jedno zdanie z ceną i przycisk. Zakres, zastrzeżenia i wyliczenie opłacalności stoją wyłącznie na landingu.
- Wymiary po przebudowie: **109 px wysokości na 1440 px** i 151 px na 390 px (pomiar Playwrightem na `/podglad-oferty`, strona tymczasowa, usunięta).
- **Ani render urządzenia, ani zdjęcie serwisu nie były tym, o co chodziło** (dwie tury uwag). Docelowo: **artefakty 3D** — trzy faktury przekreślone limonkową kreską, czerń z limonką Zebry, mokra posadzka z odbiciami. Zero sprzętu w kadrze.
- **Grafika: Higgsfield gpt_image_2 (2K, 21:9)**, artefakty ustawione w JEDNYM POZIOMYM RZĘDZIE. Pierwsza wersja miała je w kwadratowym gronie i nie dawała się wpasować w pas 10,7:1 bez ucinania — układ musi mieć proporcję zbliżoną do docelowego kadru, inaczej żaden crop nie pomoże.
- **Animacja: Google Gemini Omni Flash 1.1** (`gemini_omni_flash_1_1`, image-to-video, 5 s, 1080p). Model podpowiadał presety („IN THE DARK", „3D RENDER") — odrzucane przez `declined_preset_id`, bo narzucały własną stylizację. **Materiał 21:9 podłożony pod czarne płótno 16:9 przed wysłaniem**, bo model przyjmuje tylko 16:9 i sam uciąłby skrajne ikony.
- **Ruch to światło, nie skakanie** (uwaga użytkownika: „raczej się mienić niż podskakiwać"). Pierwsze podejście kazało obiektom unosić się i obracać — model dokładał do tego najazd kamery i ikony wypadały poza kadr. Prompt przepisany na: obiekty stoją nieruchomo, wędruje po nich odblask, limonkowe krawędzie oddychają, odbicia na posadzce falują. Sprawdzone pomiarem: bounding box treści jest w całym ujęciu stabilny (1092 × 665 px, dryf poniżej 15 px), a RMSE między klatkami 14% — czyli ruch jest, ale kadr stoi.
- **Pas jest pełnokadrowy, artefakty duże i celowo ucięte u góry i u dołu.**
- **Rząd nie mieścił się w podziale pasa** (uwaga użytkownika: „nie widać ani klucza, ani śrubokręta"). Klucz stał dokładnie na granicy czarnego woalu pod tekstem, a śrubokręt pod przyciskiem. Rachunek: tekst potrzebuje ~30% szerokości, pięć artefaktów ~46%, CTA ~18%. Rozwiązanie: klatka **dopełniona czernią po prawej** (`pad=2370:1080`) przed wycięciem paska — cały rząd przesuwa się w lewo, ląduje w oknie 35–81% i oba skrajne narzędzia wychodzą spod woalu i spod przycisku. Pasek 2370 × 221 skalowany do 2336 × 218.
- **Rząd rozciągnięty od tekstu do przycisku i wygaszony** (uwaga użytkownika). Okno kadru zwężone z 2370 do 2022 px, więc artefakty zajmują teraz 31–84% szerokości zamiast 35–81% i są o 17% większe. Pasek podniesiony w kadrze (y 288 zamiast 371), żeby widać było czub klucza i grot śrubokrętu — wcześniej oba były ucięte i te dwa narzędzia gubiły się wśród pozostałych. Do tego krycie 0,35 na warstwie grafiki — artefakty mają być tłem, nie bohaterem; przy pełnej jasności konkurowały z nagłówkiem i z przyciskiem. Krycie wpisane stylem, nie klasą: `opacity-55` nie istnieje w skali Tailwinda i klasa by się nie wygenerowała.
- Przy okazji skrócony podpis („Transport, diagnostyka i robocizna w cenie. 599 zł netto.") — dłuższa wersja wchodziła na klucz. Woal przepisany na szybki zjazd (czerń do 29%, 0,08 od 48%) zamiast łagodnego, żeby artefakty nie były przygaszone na całej długości.
- **Pętla bez cięcia: boomerang** — materiał sklejony z własnym odwróceniem (`split`/`reverse`/`concat`), 10 s, bez dźwięku, 328 kB. Zwykłe zapętlenie skakałoby na styku.
- **Research wytycznych 2026** (na życzenie użytkownika) i co z niego weszło: jedno CTA, w kolorze niewystępującym nigdzie indziej w banerze (biały przycisk przy limonkowych artefaktach); CTA na końcu ścieżki wzroku, czyli **przy prawej krawędzi**; hierarchia nagłówek → dowód → CTA; „blink test" — po sekundzie widać markę, ofertę i następny krok. Ruch w 2026 ma być funkcjonalny i powściągliwy, stąd oddychające światło zamiast animacji przyciągającej uwagę na siłę.
- **CTA przerobiony na materiał artefaktów** (dwie tury uwag: najpierw biała pigułka „wyglądała jak wklejony element UI", potem płaska obwódka „trochę lepiej, ale nie do końca"). Diagnoza drugiej wersji: limonkowa krawędź przycisku ginęła w limonkowym neonie ikon, a płaski kształt odstawał od obiektów 3D.
- Ostatecznie **płaska limonkowa pigułka** (#A8F000, czarny tekst, na hover jaśniejsza limonka). Po drodze była wersja z materiałem 3D — gradientem, rozbłyskiem i poświatą — ale po przygaszeniu artefaktów do 35% przycisk nie musi już z nimi konkurować, a płaska plama koloru jest czytelniejsza.
- **Pod przyciskiem leży wyciemniona płyta** (radialny gradient przy prawej krawędzi, tylko desktop). Bez niej artefakty przechodziły pod CTA i zjadały jego krawędź — sam dobór koloru przycisku tego nie rozwiązywał.
- Krzyżyk stoi NAD przyciskiem, nie obok — przy wysokości 109 px mijają się pionowo, więc CTA może stać przy samej krawędzi.
- Woal nad grafiką jest osobny dla telefonu (sięga do 52% szerokości) i dla desktopu (do 34%) — na wąskim ekranie tekst zajmuje ponad połowę pasa i bez tego wchodził na stoper.
- **„×" ukrywa ofertę na 30 dni, nie na zawsze** (poprawka po teście użytkownika: kliknął krzyżyk i baner zniknął bezpowrotnie). Kontrakt jest ofertą stałą, a klient zamyka pasek najczęściej dlatego, że akurat przyszedł zgłosić naprawę, a nie dlatego, że temat go nie interesuje. W localStorage leży termin powrotu, klucz per `id` oferty — nowa kampania pokaże się od razu, a zapisy w starym formacie (data ISO) są ignorowane, więc baner wraca bez czyszczenia przeglądarki.

- **Klucz płaski wymieniony na pęsetę** (uwaga użytkownika: „takiego klucza to ja nie używam"). Klucz płaski to narzędzie mechanika, nie serwisu drukarek. Porównane dwa zamienniki — pęseta i rolka etykiet; użytkownik wybrał pęsetę. Rząd wygenerowany od nowa i przeanimowany tym samym promptem, współrzędne kadru przeliczone (pad do 2240, wycięcie 1987 × 185 przy y 318 — kadr dobierany na próbach, bo pęseta i wkrętak są rozpoznawalne tylko po czubkach: przy y 385 widać było same trzonki), bo obiekty stoją w innym miejscu klatki niż poprzednio.

## 2026-09-01 — zgoda na reklamy nie docierała do Google Ads
- **Kampania serwisowa w Ads pokazywała zero konwersji od miesięcy**, choć GA4 miał z ruchu płatnego (90 dni) 70 wiadomości na czacie, 5 formularzy naprawy i 3 kliknięcia w telefon. Konfiguracja po stronie Google była w porządku: powiązanie GA4 ↔ Ads od 4.07 na właściwym koncie, auto-tagowanie włączone, cztery akcje konwersji aktywne i oznaczone jako główne, okno 90 dni, wskazują na usługę 517189747.
- **Blokadą był baner cookies.** `gtag('consent','update')` podnosił tylko `analytics_storage` i `ad_storage`, a layout ustawia domyślnie wszystko na „denied". Bez **`ad_user_data`** GA4 nie ma prawa wyeksportować konwersji do Ads — mierzył je u siebie i na tym koniec.
- **Druga rzecz z tego samego pliku:** zgoda z poprzedniej wizyty wracała tylko do stanu komponentu, nigdy do gtag. Powracający użytkownik zostawał na „denied", mimo że wcześniej kliknął „Akceptuję" — więc jego wizyt nie mierzył nawet Analytics.
- `components/CookieBanner.tsx`: jedna funkcja `przekazZgodeDoGtag()` wysyła komplet zgód reklamowych (`ad_storage`, `ad_user_data`, `ad_personalization`, `personalization_storage`) i jest wywoływana zarówno po kliknięciu w baner, jak i przy każdym wejściu z zapisaną zgodą.
- Sprawdzone na produkcji przez przeglądarkę: po „Akceptuj" leci `consent update` z `ad_user_data: granted`, a przy kolejnym wejściu ten sam update powtarza się bez pokazywania banera. **Dane historyczne nie wrócą — licznik startuje od wdrożenia.** Pierwsze konwersje w Ads powinny pojawić się w 24–48 h od pierwszego zgłoszenia z reklamy.
- Dla porównania: takma.com.pl nie ma trybu zgody w ogóle, dlatego tam import konwersji działał od zawsze (50 klik_tel, 45 klik_mail, 18 zakupów w 90 dni).

- **Zestaw ikon zastąpiony przekreślonym rachunkiem.** Po pęsecie użytkownik stwierdził, że rendery przestały mu się podobać w ogóle — problem był systemowy, nie w doborze narzędzia: rząd pięciu drobnych przedmiotów przy kryciu 0,35 czyta się jak clipart, a cienkie obiekty (pęseta, wkrętak) giną. Z burzy mózgów wybrany **przekreślony rachunek**: trzy arkusze faktur z jedną grubą limonkową kreską w poprzek. Jeden duży, prosty kształt zamiast pięciu drobiazgów, a treść dosłownie ilustruje nagłówek „bez rachunków za robociznę".
- Prompt wymusza treść dokumentu jako ABSTRAKCYJNE szare paski zamiast liter — inaczej model wpisuje w faktury bełkot, który przy powiększeniu widać.
- Kadr: pad do 2240, wycięcie 1962 × 183 przy y 330 (kreska przecina pas po skosie), skala do 2336 × 218. Telefon ma osobny kadr wyśrodkowany na kresce, bo przy kadrze prawym schodziła w róg i przestawała być czytelna.

## 2026-09-01 — /kontrakt-serwisowy przepisany (hero, język, panel zakupu)
- **Hero ścięty z ~500 px do 213 px** (uwaga użytkownika: „AI slop"). Zamiast kwadratowego renderu drukarki obok tekstu — kompaktowy ciemny pas z tą samą grafiką co baner w panelu klienta (przekreślone faktury, kadr 2400 × 521 z `rach-a`). Panel i landing mówią teraz jednym językiem wizualnym, a klient, który kliknął baner, widzi na stronie docelowej to samo.
- **Treść przepisana na prosty język.** „Diagnostyka i robocizna bez dopłat" → „Praca serwisu bez dopłat. Ani za godziny przy stole, ani za samo ustalenie, co się zepsuło". „Odbiór kurierem i odesłanie w cenie" → „Kurier w obie strony". Nagłówki sekcji: „Co dostajesz", „Ile to kosztuje w praktyce", „Czego kontrakt nie obejmuje" — zamiast „Co obejmuje kontrakt", „Czy to się opłaca", „Zastrzeżenia, o których warto wiedzieć".
- **Wyrzucone teksty oczywiste** (polecenie użytkownika): „Przelewy24, BLIK albo karta. Fakturę VAT wystawiamy od razu po zaksięgowaniu wpłaty" — metody płatności klient zobaczy w kasie, a faktura VAT dla firmy jest oczywistością. Krok drugi mówi teraz to, co niesie treść: „Płacisz jeden raz. 599 zł netto za trzy lata. Potem nie dopłacasz do napraw objętych kontraktem".
- **Panel zakupu przebudowany wg wytycznych** (research: [VWO](https://vwo.com/blog/ecommerce-product-page-design/), [Ecom Design Pro](https://ecomdesignpro.com/ecommerce-trust-signals/), [Cartylabs](https://cartylabs.com/blog/shopify-product-page-best-practices/)):
  - **Kotwica NAD ceną** — „Jedna naprawa kosztuje u nas średnio 455 zł netto". Klient porównuje 599 zł z kosztem alternatywy, nie z zerem.
  - **Cena w przeliczeniu na miesiąc** (16,64 zł) liczona z ceny produktu, nie wpisana na sztywno — nie rozjedzie się po zmianie cennika.
  - **Podsumowanie koszyka z sumą** zamiast samej listy pozycji.
  - **Pasek zakupu przyklejony na dole na telefonie** (cena + „Kup kontrakt"): panel stoi na dole strony, a decyzja zapada wyżej. Przycisk przewija do formularza i ustawia kursor w polu modelu — nie dodaje od razu do koszyka, bo bez numeru seryjnego nie ma czego dodać.
  - **Pod przyciskiem tylko to, czego klient sam nie wie**: kto naprawia (autoryzowany serwis Zebry) i od kiedy liczy się ochrona. Bez „bezpiecznych płatności" i innych plakietek zaufania, które niczego nie wnoszą.

## 2026-09-01 — /kontrakt-serwisowy wg badań konwersji (2 000 stron, 2026)
Źródło twardych liczb: [digitalapplied — Landing Page Conversion: 2,000 Pages Tested in 2026](https://www.digitalapplied.com/blog/landing-page-conversion-study-2000-pages-tested-2026) oraz [Landing Page Statistics 2026](https://seosherpa.com/landing-page-statistics/). Mediana konwersji landingów w 2026 to 4,02%, górne 25% to 11,45%+.
- **Hero z jedną liczbą (+18%, najsilniejszy wzorzec w badaniu).** Nad zgięciem stoi teraz cena — 599 zł netto i 16,64 zł miesięcznie — a nie sam nagłówek. Hero urósł z 213 do 311 px i to jest świadomy koszt: bez ceny i przycisku nad zgięciem tracimy najmocniejszy pojedynczy efekt z całego badania.
- **Dokładnie JEDEN przycisk w hero.** Trzy i więcej przycisków to −8% (paraliż decyzyjny). CTA nad zgięciem daje +6%, pasek przyklejony na dole +11%, oba razem +12% — mamy oba, ale bez złudzeń, że się sumują.
- **Dowód społeczny w pierwszym oknie (+12%).** Konkretna, policzalna deklaracja zamiast ogólnika: „Autoryzowany serwis Zebry. Naprawiamy od 1999 roku, ponad 5000 urządzeń". Badanie jest tu bezlitosne: ogólniki w stylu „Trusted by thousands" działają jak brak dowodu (0%), a policzalna liczba to nawet +22%.
- **Przypomnienie CTA w połowie strony (+4%)** — po wyliczeniu opłacalności, tylko na telefonie (na desktopie panel zakupu i tak stoi obok, przyklejony).
- Czego świadomie NIE zrobiłem: **autoodtwarzane wideo w hero to −7%** (kara za LCP), więc animacja z banera zostaje w panelu, a na landingu jest statyczna grafika. **Zdjęcia stockowe to −11%** — nasz render jest brandowy i związany z ofertą, więc zostaje.
- Formularz ma 2 pola (model + numer seryjny). W badaniu 1 pole = 12,4% konwersji, 4 pola = poziom odniesienia, 6+ pól = 3,1%. Dwóch pól nie da się zmniejszyć — bez numeru seryjnego kontrakt nie ma do czego się przypiąć.
- **Największa niewykorzystana dźwignia: opinia klienta z imieniem, nazwiskiem i zdjęciem (+14%).** Nie dodaję, bo nie mam prawdziwej — do zebrania od klienta, który ma już kontrakt.
- **Cena tylko raz w polu widzenia** (uwaga użytkownika: „w zasięgu wzroku masz 2x cenę"). Hero i panel zakupu powtarzały to samo — 599 zł netto i 16,64 zł miesięcznie — w jednym oknie. Duża cena została w hero (realizuje wzorzec „single-stat", +18%), a z góry panelu zniknęła całkiem.
- **Panel zakupu przebudowany**: nagłówek „Kup kontrakt na swoją drukarkę" mówi, po co ten box istnieje, potem zdanie o jednym urządzeniu (najczęstsze pytanie klienta), dwa pola, a cena stoi w wierszu TUŻ NAD przyciskiem — tam jej klient szuka, podejmując decyzję, a nie na górze panelu. Kotwica 455 zł została wyłącznie w treści strony, więc nie dubluje się z niczym.
- **Poprawione zdanie o ochronie** (uwaga użytkownika: „gramatycznie i składniowo dramat"). Było: „Ochrona rusza w dniu zaksięgowania wpłaty i trwa do dnia co do daty trzy lata później". Jest: „Ochrona zaczyna się w dniu zaksięgowania wpłaty i trwa trzy lata".

## 2026-09-01 — /kontrakt-serwisowy: ponowny audyt na buildzie (iPhone 13 + 1440 px)
- **Cena była dwa razy w jednym oknie na OBU ekranach**, mimo wcześniejszej poprawki: na telefonie hero (599 zł) i przyklejony pasek (599,00 zł) z dwoma przyciskami „Kup kontrakt" naraz; na desktopie hero i wiersz ceny w panelu mieściły się razem w 900 px. Zasada po zmianie: **cena jest w jednym miejscu w polu widzenia**. `ContractPurchasePanel` obserwuje (IntersectionObserver) hero `#hero-kontrakt` i panel `#panel-zakupu`: wiersz ceny w panelu jest ukryty, dopóki widać hero; pasek na telefonie wjeżdża dopiero, gdy nie widać ani hero, ani formularza. Zmierzone: góra strony — jedna cena (hero); 700 px — jedna cena (pasek albo panel); przy formularzu — pasek schowany.
- **Cena z bazy zamiast wpisanej na sztywno.** Hero, krok „Płacisz jeden raz", akapit o kosztach i title strony liczyły się z literału „599"; panel brał cenę z Supabase. Teraz wszystko idzie z `product.price` (`zl()` formatuje „599" i „16,64"). Metadane przeniesione z `layout.tsx` do `generateMetadata` w `page.tsx`, pobranie produktu opakowane w `cache()` z Reacta, więc Supabase dostaje jedno zapytanie na żądanie.
- **Hero na telefonie dostał kadr mobilny** (`/oferty/kontrakt-serwisowy-mobile.webp`, ten sam co baner w panelu) z pionowym woalem 0,85 → 0,35. Szeroki kadr 2400 × 521 z `object-right` pokazywał na 390 px czarną plamę na 59% pierwszego okna.
- **„Co dostajesz" na telefonie to lista w jednej ramce**, nie sześć kart; desktop bez zmian. Strona krótsza o ~200 px (4539 → 4335), formularz z 2567 na 2386 px. Mniej niż liczyłem — treść sześciu punktów po 2–3 linie na 390 px i tak zajmuje swoje.
- **Przycisk w hero i przypomnienie w połowie strony** to teraz `KontraktCtaLink` (klient): przewija i ustawia kursor w polu modelu, tak jak pasek. Dystans pod pasek przeniesiony z komponentu na koniec `main`, więc nie rozpycha już odstępu przed zdaniem o flocie.
- **Schema `Product` + `Offer`** (cena brutto jako `price`, netto w `UnitPriceSpecification` z `valueAddedTaxIncluded: false`, sprzedawca jak na innych landingach) i `BreadcrumbList`. Link „Kontrakt serwisowy" w kolumnie Serwis w stopce — wcześniej strona była linkowana tylko z koszyka, kategorii sklepu, banera w panelu i newslettera.
- 404 w konsoli to `/_vercel/insights/script.js` — skrypt Vercel Analytics, którego nie ma lokalnie. Na produkcji nie występuje.
- Zostaje w prozie: krok 2 i akapit o kosztach wymieniają cenę w tekście, więc na desktopie po przewinięciu widać ją w treści i w panelu jednocześnie. To zdania, nie wyeksponowana liczba — świadomie zostawione.

## 2026-09-01 — druga oferta w panelu: promocja na etykiety (ZipShip)
- Źródło treści: `takma/src/data/promotions.ts`, wpis `zebra-materialy-eksploatacyjne`, landing i formularz na `https://www.takma.com.pl/promocje/zebra-materialy-eksploatacyjne` (pas linkuje na zewnątrz, cross-linking między serwisami). **Na froncie wyłącznie „do 15%" od naszych cen** — rabat partnerski z PartnerConnect jest poufny i nie ma prawa się pojawić.
- Nowe pole `koniec` w `OfertaPanelu` (RRRR-MM-DD) i funkcja `ofertaAktywna()` — pas znika sam po ostatnim dniu promocji (31.12.2026), kontrakt bez daty jest ofertą stałą. `PasOfertowy` filtruje po dacie przed sprawdzeniem ukrycia.
- **Grafika: Higgsfield `gpt_image_2` 2K 21:9 z referencją kafla `takma/public/images/promo-card-materialy.webp`** (ta sama czerń, limonkowa poświata, mokra posadzka). Trzy rolki etykiet w JEDNYM POZIOMYM RZĘDZIE, prawa rozwija pas etykiet po posadzce, zero tekstu i zero sprzętu. Dwa warianty: A — rolki na boku z widocznymi gilzami; B — rolki stojące, wyglądały jak papier toaletowy. Wybrany A.
- Kadr desktop: `crop=2688:251:0:390` → 2336 × 218 (rolki na 37–80% szerokości, gilzy w całości, góra i dół ucięte). Kadr mobilny: `pad=3188:1317:500:165` → 1100 × 455 (rolki na 47–83%, żeby wyjść spod woalu do 52%). Zmierzone na `/podglad-oferty` (tymczasowa strona, usunięta): 109 px na 1440, 170 px na 390.
- **Animacja: `gemini_omni_flash_1_1`, `mode: image-to-video`** (parametr `mode` jest wymagany — bez niego 422; preset „IN THE DARK" odrzucony przez `declined_preset_id`). Materiał 21:9 podłożony pod czarne 16:9 (1920 × 823 na 1920 × 1080, offset y 128).
- **Pierwsza generacja odrzucona**: model zamienił poświatę w zielony dym, środkowa rolka błysnęła na biało w 2,5 s, a linia podłogi znikła. Prompt przepisany z jawnym zakazem dymu, mgły, haze, cząstek, błysków i z nakazem „scena wygląda jak to samo zdjęcie". Drugi wariant A: rolki nieruchome, poświata oddycha, linia zostaje, RMSE ostatnia/pierwsza klatka 0,013. Wariant B rozjechał się na końcu (RMSE 0,255).
- Pas wideo: `crop=1920:180:0:406` → 2336 × 218, boomerang (split/reverse/concat), 10 s, bez dźwięku, CRF 30, **213 kB**. Pliki: `public/oferty/etykiety-promocja{,.mobile}.webp`, `etykiety-promocja.mp4`.
- **Kolejność w `OFERTY_PANELU` = priorytet.** Kontrakt stoi pierwszy, więc promocję etykiet zobaczy tylko klient, który kontrakt już ma albo go zamknął na 30 dni. Kto ma widzieć którą ofertę — do ustalenia przed pushem (na razie bez targetowania).

## 2026-09-01 — kontrakt w panelu: odbiorcy = wycena naprawy drukarki (PUSH cd7d6a1)
- Decyzja użytkownika: baner kontraktu pokazuje się klientowi, który zgłosił drukarkę Zebra, w chwili gdy serwisant wystawi wycenę — i tak ma być stale dla każdego kolejnego zgłoszenia.
- Reguła `pokazGdy` w `OfertaPanelu` liczona z listy zgłoszeń klienta: `device_type === 'drukarka'` (pole z selecta formularza; wartości: drukarka, terminal, skaner, tablet, akcesoria, inne) i status w `['wycena', 'w_naprawie', 'zakonczone']` — od wystawienia wyceny do wysyłki. Przed wyceną (nowe, odbior_od_klienta, odebrane, diagnoza) baner się nie pokazuje, po `wyslane` znika.
- `PasOfertowy` dostaje `naprawy` w propsie: dashboard podaje całą listę z `/api/repairs`, strona zgłoszenia jedno zgłoszenie. Efekt zależy od klucza `device_type:status`, nie od tożsamości tablicy — strona zgłoszenia renderuje się często (czat) i bez tego pas odpytywałby `/api/service-contracts` przy każdym renderze.
- Pas dodatkowo na `/panel/naprawa/[id]` pod osią statusów i blokiem „Wymagana akcja" (tam klient patrzy na kwotę wyceny). Wrapper z `empty:hidden`, żeby pusty pas nie zostawiał odstępu.
- Sprawdzone Playwrightem na tymczasowej stronie z pięcioma przypadkami (drukarka+wycena → pas; drukarka+diagnoza, terminal+wycena, drukarka+wyslane, brak zgłoszeń → brak pasa). Strona usunięta.
- Stan bazy w dniu wdrożenia: 5 zgłoszeń drukarek przed wyceną (1 nowe, 4 odbiór od klienta) — te zobaczą baner, gdy dostaną wycenę; 8 zgłoszeń drukarek z wyceną i 1 w naprawie — ci widzą baner od razu.
- **Baner etykiet NIE poszedł z tym pushem** — czeka lokalnie (niezacommitowany) na decyzję, komu go pokazywać; bez reguły widziałby go każdy klient, który nie łapie się na kontrakt.

## 2026-09-02 — etykiety: nie baner, tylko mailing; testówki wysłane
- Decyzja użytkownika: klient po odbiorze drukarki nie wraca do panelu, więc promocja etykiet idzie mailem, nie pasem w panelu. Baner etykiet (grafika, wideo, wpis w `OFERTY_PANELU` z polem `koniec`) zostaje lokalnie, niezacommitowany.
- `scripts/newsletter-etykiety-test.mts` — wysyłka testowa trzech wariantów szablonu z `lib/email/newsletter-etykiety.ts` (GK420d → ZD421d, GX430t → ZD421t, ZD421t → kontrakt) na jakub.tiuchty@gmail.com przez piaskownicę Resend (`onboarding@resend.dev`). Uruchomienie: `set -a; source .env.local; set +a; node scripts/newsletter-etykiety-test.mts` — Node 22 czyta `.ts` bez tsx, import szablonu ścieżką względną z rozszerzeniem. Linki: „Odbierz rabat" → `/kontakt?temat=etykiety`, sklep → takma `/etykiety-termiczne-zebra`, wypis → `/wypisz` (strona jeszcze NIE istnieje). Grafiki z produkcji (`/newsletter/hero-etykiety.jpeg`, następcy, kontrakt — wszystkie 200).
- **Zgoda marketingowa** siedzi w `user_metadata.marketing_consent` konta auth (nie w `profiles`): 201 użytkowników — 16 tak, 62 nie, 123 bez zapisu. Klientów z odesłaną drukarką: 87. Wyjaśnione użytkownikowi: art. 398 PKE wymaga uprzedniej zgody także od firm i dotychczasowych klientów, kara UKE do 3% przychodu; do tego skargi „spam" psują reputację domeny, z której idą maile o wycenach. Zaproponowane: kartka A5 z QR w paczce z naprawioną drukarką (papier bez zgody), blok w mailu „przesyłka wysłana", zbieranie zgody w profilu i w tym mailu, pełny mailing cronem tylko do zgód. Decyzja użytkownika w toku.
## 2026-09-02 — kontrakt: GA4 + numer seryjny widoczny przy ponownym zgłoszeniu
- **GA4**: `PasOfertowy` wysyła `oferta_panel_wyswietlenie` (raz na wejście) i `oferta_panel_klik` (z `oferta_id` i `cel`). Strona sukcesu zamówienia wysyła `purchase` (transaction_id, value brutto, PLN) i — gdy w zamówieniu są kontrakty — `kontrakt_zakup` (value netto kontraktów, `liczba_kontraktow`). Blokada powtórki przez `sessionStorage` na numer zamówienia, bo klient odświeża stronę sukcesu. `/api/shop/p24/status` zwraca dodatkowo `kontrakty_count` i `kontrakty_netto` (z `contractItemsOf`). Stary Stripe `verify` nie ruszony — nieużywany. **Oznaczenie `kontrakt_zakup` jako zdarzenia kluczowego robi się w GA4 ręcznie** (Administracja → Zdarzenia → przełącznik); konto serwisowe w repo to Vertex RAG, bez dostępu do GA4.
- **Kontrakt po numerze seryjnym — dopinany przy ODCZYCIE, nie przy zapisie.** `kontraktDlaSerialu()` w `lib/service-contracts.ts`: szuka po samym numerze (nie po kliencie — kontrakt kupuje sklep, zgłasza konto w panelu, często inny e-mail), tylko `active` z `ends_at >= dziś`, numer znormalizowany (bez spacji, wielkie litery). Bez nowej kolumny w `repair_requests`: zgłoszenie sprzed zakupu kontraktu też go pokaże, a wygasły zniknie sam.
- Gdzie widać: (1) `NewRepairModal` — po wpisaniu numeru (≥5 znaków, 500 ms) odpytuje `/api/service-contracts/check?serial=` (tylko zalogowani) i pokazuje zielony box „objęta kontraktem … do …"; (2) `/panel/naprawa/[id]` — box w nagłówku mobilnym i w karcie urządzenia; (3) `/admin/zgloszenie/[id]` — wyróżniona ramka przy numerze seryjnym z datami i zakresem („bez opłat za robociznę, diagnostykę i transport; części 40% taniej"), żeby serwisant zobaczył to PRZED wyceną. API `/api/repairs/[id]` i `/api/admin/repairs/[id]` dokładają `kontrakt` do obiektu zgłoszenia.
...
## 2026-09-02 — audyt 10 kart drukarek biurkowych (88/100) i poprawki
- Skan: parse_html + content_quality na 10 kartach ZD, Lighthouse mobile na ZD421t (perf 94, a11y 89, SEO 100). Szablon spójny: ProductGroup z 6 wariantami Product (sku, mpn, Offer, InStock), WebPage z dateModified, FAQ, canonical także dla `?pn=`. Raport w pamięci `seo-audit-karty-biurkowe.md`.
- **Meta z bazy (`products.meta_title/meta_description`), nie z kodu.** Title był 64 zn. („… — cena i dostępność | Serwis Zebra"), description 158–188 zn. na 8 z 10 kart. PATCH przez REST kluczem serwisowym: title → „Drukarka etykiet Zebra ZD421t — cena i dostępność | TAKMA" (57 zn.), description skrócone do 142–155 zn. z zachowaniem faktów. **Pułapka**: `slug.upper()` dało „ZD421T" — sufiks d/t musi zostać małą literą, poprawione drugim PATCH-em.
- **A11y 89 → 100** (Lighthouse na buildzie): kontrast `text-gray-400` → `text-gray-500` (etykiety 11 px w opisie i akcesoriach), chip „Na zamówienie" `text-gray-500` → `600`, komórka „Stan" `text-green-600` → `700` (3,29 → 5,0:1); `ShopSubheader` — link logo `aria-label="Strona główna"`, koszyk `aria-label` z liczbą sztuk; `DeviceVariantsTable` — aria-label przycisku zaczyna się od widocznego tekstu („Dodaj do koszyka: …"), bo WCAG 2.5.3 wymagał zgodności; stopka — dekoracyjne „SERWIS ZEBRA" z `h2` na `div aria-hidden` (wchodziło do konspektu KAŻDEJ strony), kolumny `h4` → `h3`; link do polityki w banerze cookies z podkreśleniem (był tylko kolorem, 1,46:1 wobec tekstu).
...
## 2026-09-02 — karta Zebra ZT421 (pierwsza w klasie przemysłowej)
- Wg skilla `karta-produktu`, kolejność: pliki → baza `is_active:false` → build i weryfikacja → push → aktywacja po deployu.
- **Dane producenta** z karty ZT400 (`zt400-series-spec-sheet-en-us.pdf`, pdftotext): druk 168 mm, 305 mm/s, 203/300 dpi (600 tylko ZT411), długość etykiety 2591/1143 mm, etykiety 51–178 mm (51–171 z nawijakiem), rolka 203 mm/76 mm, taśma 450 m 51–174 mm gilza 25 mm, 256 MB SDRAM/512 MB Flash, USB 2.0 + 2×USB Host + Ethernet + RS-232 + BT 4.1, opcje Wi-Fi ac/ax, drugi Ethernet, równoległy, aplikator, RFID UHF, 5–40°C TT / 0–40°C DT, 495×336×324 mm (500×338×425 z nawijakiem), **18,14 kg** (25,08 z nawijakiem). **Takma podaje 14,2 kg — błędnie**; grubość materiału pominięta (brak w karcie).
- **PN-y zweryfikowane `product-stock`**: istnieje 7 z 12 kandydatów — 62/63 × T0E (standard), T2E (gilotyna), T4E (nawijak etykiet) + 62-T0EC (Wi-Fi). T1E (odklejak), T3E (nawijak podkładu) i 63-T0EC NIE istnieją u dystrybutora. Najtańszy ZT42162-T0E0000Z 10 523,36 netto, PL 23 szt.
- Baza: `products` id `0caf99f7…`, slug `zebra-zt421`, klasa `przemyslowe`, 7 wariantów z cechami Rodzaj druku/Rozdzielczość/Łączność (Ethernet|Wi-Fi)/Wyposażenie (Standard|Gilotyna|Nawijak etykiet), meta_title 56 zn. (`| TAKMA`), description 147 zn. Zdjęcia: 3 rendery z takmy (paleta P → spłaszczone na biel → cwebp q90, 84–111 kB), `_1` trzyma konwencję (front w prawo).
- `lib/device-content.tsx`: wpis `zebra-zt421` (wSkrocie z A5 148 × 210 mm jako argumentem, 3 akapity, osie: jakość/wyposażenie/łączność, 5 FAQ z linkami 200, 23 wiersze spec). Link w opisie ZT411 „drukarka przemysłowa 168 mm" przekierowany z klasy na kartę ZT421.
- **Strona klasy `/sklep/drukarki-etykiet/przemyslowe` przepisana z zapowiedzi (`KlasaNaZamowienie`) na pełny wzorzec** półprzemysłowej: katalog z filtrem, notka „ZT510/610/620 na zamówienie", 8 faktów, tabela ZT411 vs ZT421, 6 FAQ, ItemList z Product+Offer, og:image + twitter z `/klasy/przemyslowe.jpg`.
- Checklist: sitemap.ts (0.9), sitemap-images.xml (3 zdjęcia), llms.txt (nowa sekcja „Drukarki przemysłowe"), `SPRZEDAWANE_MODELE` w instrukcjach (`/instrukcje/zebra-zt421` istnieje, 200).
- Weryfikacja na buildzie (produkt włączony na 3 minuty i wyłączony): 200 karta i klasa, 7 PN-ów z cenami SSR, JSON-LD 5/5 i 4/4, ProductGroup 7 wariantów z cenami, baner materiałów TT, zero poziomego scrolla 1440/390, zero zepsutych obrazów, `test-filtr-wariantow` zielony.
- **Pułapka**: `[slug]/page.tsx` pobiera tylko `is_active=eq.true`, więc weryfikacja karty na buildzie wymaga chwilowego włączenia produktu (klasa `przemyslowe` na produkcji była wtedy jeszcze zapowiedzią, więc kafelek nigdzie nie wyciekł).
- PO DEPLOYU: `is_active:true`, ręczny `stock-sync` (Bearer CRON_SECRET), sprawdzenie produkcji.
- Wdrożone 20:15: deploy potwierdzony (zdjęcie 200), produkt włączony, karta i klasa na produkcji 200 z 7 cenami. Ręczny `stock-sync` przekracza 120 s (przelicza cały katalog) — cache 7 PN-ów i tak świeży z `product-stock`.

## 2026-09-02 — audyt SEO karty ZT421 (88/100) i poprawki
- `/seo-audit` na jednym adresie: claude-seo (render_page, parse_html, content_quality) + Lighthouse 12 mobile/desktop na produkcji. Raport, plan i PDF (10 stron) w scratchpadzie sesji i u użytkownika; notatka w pamięci `seo-audit-zt421.md`. **PDF**: WeasyPrint ze skilla nie ma bibliotek systemowych — `google_report.py --format html` + Playwright `page.pdf()`. Pułapka: w `audit-data.json` polskie cytaty „…" z prostym cudzysłowem zamykającym psują JSON — zamieniać na „…”.
- Wyniki: Technical 82, Content 92, On-page 95, Schema 88, Performance 78 (mobile 87 / desktop 74, LCP 3,7 s mobile z Load Delay 2,2 s), GEO 92, Images 90.
- **Regresja z wczoraj naprawiona**: `aria-label="Strona główna"` na linku „Start" w `ShopSubheader` psuł WCAG 2.5.3 na desktopie → `span.sr-only sm:not-sr-only`, bez aria-label (bfe8fa4).
- **Poprawki z planu**: (1) nagłówki bezpieczeństwa w `next.config.js` `headers()`: nosniff, X-Frame-Options SAMEORIGIN, Referrer-Policy strict-origin-when-cross-origin, Permissions-Policy — cały serwis, bez CSP (inline skrypty zgód/GTM wymagałyby nonce); (2) Clarity z inline `<script>` w `<head>` na `next/script` `lazyOnload` — konkurował o łącze z obrazem LCP; (3) `[slug]/page.tsx`: akcesoria i stany przez `Promise.all` (lokalnie TTFB 250–300 ms); (4) `Header`: `prefetch={false}` na linku logo — chunk strony głównej (26 KiB) dociągał się na każdej karcie; (5) `BanerMaterialow` `sizes` bez `vw` (`(max-width: 640px) 640px, 256px`).
- **Pułapka next/image**: gdy `sizes` zawiera jednostkę `vw`, srcset buduje się wyłącznie z `deviceSizes` (od 640 px) i `imageSizes` są ignorowane — kafel 256 px dostawał obraz 640 px mimo `sizes="…, 256px"`. Zapis w px odblokowuje 256/384.
- Do sprawdzenia po deployu: nagłówki na produkcji, LCP mobile (cel < 2,5 s), CrUX po 28 dniach.

## 2026-09-02 — karta Zebra ZT510 (druga w klasie przemysłowej)
- Dane z karty technicznej producenta (`zt510-technical-spec-sheet-a4-en-us.pdf`, wyszukana WebSearch, bo stary URL 404): druk 104 mm, **305 mm/s przy 203 dpi, 254 mm/s przy 300 dpi**, etykieta do 991 mm (ciągły 3810/2540 mm), etykiety 20–114 mm, grubość 0,076–0,305 mm (0,23 z gilotyną), rolka 203/76 mm, taśma 450 m 20–110 mm gilza 25,4, 2 GB Flash/512 MB SDRAM, **LCD z klawiaturą (nie dotyk)**, Gigabit Ethernet + USB 2.0 + RS-232 + BT LE + NFC, opcje Wi-Fi ac/BT4.0, równoległy, USB Host, drugi Ethernet; **gilotyna wyklucza nawijak/odklejak**; 512×268×396 mm, **22,7 kg**; **bez RFID i aplikatora, bez EPL**. Takma podaje to samo poza „Bluetooth LE standard" — zgodne.
- **PN-y**: 8 z 12 kandydatów istnieje — 42/43 × T0E, T0EC (Wi-Fi), **T1E = gilotyna, T2E = nawijak** (inne kodowanie niż ZT411, gdzie T1E = odklejak, T2E = gilotyna). T3E/T4E nie istnieją. Najtańszy ZT51042-T0E0000Z 8 277,62 netto (PL 0, DE 81).
- Baza: id `0a1c045e…`, klasa `przemyslowe`, 8 wariantów; Wyposażenie: Standard | Gilotyna | **Nawijak** (nowa wartość filtra — nawijak ZT510 jest uniwersalny: pełna rolka albo odklejak z nawijakiem podkładu). meta_title 56, description 152. Zdjęcia: `_1` i `_2` paleta P → biel, `_3` RGBA; 29–136 kB.
- Treść: wSkrocie z pozycjonowaniem „mechanika na trzy zmiany, nie dodatki", opis z linkami do ZT411 (dodatki) i ZT421 (168 mm), osie jakość/wyposażenie/łączność, 5 FAQ (w tym „czy ma ekran dotykowy" → link do instrukcji ZT510, 200), spec 24 wiersze.
...
- Druga runda (uwaga: „indywidualne ikony, w hero grafika komiksowo-wektorowa"): hero w siatce `lg:grid-cols-[1.1fr_1fr]` z ilustracją `public/o-nas/hero-warsztat.jpg` (gpt_image_2 16:9 2k, refy: styl `klasy/biurkowe.jpg` + `zd421t_1.webp`; prompt z „dark blue-grey background", żeby wtopiła się w pas bez ramki). Korzyści autoryzacji: własne ikony `korzysc-{czesci,gwarancja,certyfikat,wsparcie}.png` (karton z kołem zębatym, tarcza z ptaszkiem, odznaka z gwiazdą, słuchawka z błyskawicą) na białych kółkach 32 px.
- Trzecia runda („zdjęcie na całe hero, urządzenie 1:1, bez człowieka"): ilustracja `public/o-nas/hero-urzadzenia.jpg` — 4 referencje (styl + `zd421t_1`, `zt411_1`, `zq521_1` spłaszczone na biel), prompt „NO people… devices reproduced EXACTLY 1:1… LEFT HALF empty dark navy wall". Z dwóch wariantów wybrany ten z wiernym, ciemnym ZT411 (drugi rozjaśnił obudowę). Desktop: `Image fill object-right` pod całym pasem + gradient `from-gray-900 via-gray-900/85 to-gray-900/10`; `lg:hidden` blok z tą samą ilustracją pod tekstem na telefonie (na tle tekst lądował na narzędziach, a urządzenia wypadały z kadru). `hero-warsztat.jpg` (z technikiem) usunięty.
- Czwarta runda: „usuń urządzenia, daj artefakty/abstrakcję mówiącą: autoryzowany serwis Zebry" → `public/o-nas/hero-abstrakcja.jpg` (gpt_image_2 16:9 2k, tylko referencja stylu): tarcza z ptaszkiem, odznaka ze wstążką, klucz, koło zębate, etykiety z kodami, zebrowe pasy w limonce; lewe 45% puste pod tekst. Bez urządzeń, ludzi, tekstu i logo Zebry (znak towarowy). Tło pasa `bg-[#011022]` = kolor tła grafiki (zmierzony `magick … -format "#%[hex:…]"`), gradient `from-[#011022] via-[#011022]/60`. Na telefonie `object-cover object-right`. `hero-urzadzenia.jpg` usunięty.
- Szósta runda: „oś czasu jako animowany timeline" → `components/OsCzasu.tsx` ('use client'): IntersectionObserver (threshold 0,25, jednorazowo) przełącza `widoczna`; dwie linie (pionowa `lg:hidden` `scaleY`, pozioma `lg:block` `scaleX`, 1,2 s), punkty `scale(0→1)` i karty `translateY(16px→0)` z opóźnieniem `300 + i·220 ms`, ostatni punkt limonkowy z `animate-ping`. Bez bibliotek; `prefers-reduced-motion` = wszystko od razu. Siatka `lg:grid-cols-4`, poniżej jedna kolumna (dwie kolumny na tablecie gubiły pionową linię). Zrzuty start/koniec animacji na buildzie.
- Pasek statystyk („to też mi się nie podoba") → `components/LicznikiStat.tsx`: `ul` z `divide-x` (desktop) / `divide-y` (telefon), pole = ikona 28 px w kółku 48 px (certyfikat, karton, lupa z kluczem, uścisk dłoni) + liczba `text-3xl tabular-nums` odliczana od zera (`requestAnimationFrame`, 1,8 s, ease-out sześcienny, `Intl.NumberFormat('pl-PL')`) + sufiks „+"/„lat" w limonce + podpis. Dane jako `{liczba, sufiks, label, icon}`. Sekcja `py-4 sm:py-6` — niższa niż była. ODRZUCONE („za dużo animacji, ikony nie pasują"): komponent usunięty. Wersja końcowa: statyczna limonkowa wstęga `bg-[#A8F000]` pod hero, `dl` 2/4 kolumny z `divide-gray-900/15`, liczby `text-3xl sm:text-4xl` w czerni, podpisy `text-gray-900/70`. Wniosek: liczniki i ikony zostawiać na wyraźne życzenie; użytkownik woli spokój.
- „Wolniej i coś lepszego": przebudowa — szara oś stoi od razu, po niej `linear` 3,2 s płynie limonkowy postęp (`scaleX`/`scaleY`); punkt i zapala się (obwódka szara→limonka, wypełnienie `scale 0→1` z odbiciem) w chwili dojazdu `t_i = i/(n−1)·3200 ms`, rocznik `text-3xl` nad punktem `+120 ms`, karta `+350 ms` (700 ms). Pulsowanie ostatniego punktu przez stan `pulsuje` po `setTimeout(3600)` — inline `animationDelay` na `animate-ping` pokazywało limonkowe koło od pierwszej klatki. Klatki 0,6/1,6/2,8/4,4 s sprawdzone na buildzie.
- Piąta runda: „hero o 50% mniejszy, śrubokręt zamiast klucza" → `lg:py-14`, h1 max `text-4xl`, lead bez `sm:text-lg`: 420 px zamiast ~800 na 1440. Grafika wygenerowana ponownie z „large flat-head SCREWDRIVER… NO wrench"; z dwóch wariantów lewy (kompozycja w kadrze, drugi miał ucięty prawy brzeg). Tło `#010E29` (znów zmierzone, każda generacja ma inny odcień granatu — mierzyć za każdym razem).
- „Zmniejsz, ucięte góra i dół": przy pasie 16:9 → ~3:1 `object-cover` obcinał ilustrację. Teraz `object-contain object-right` (obraz = wysokość pasa, w całości), `lg:py-32`, tło sekcji `#0f172a` + gradient `from-gray-900 via-gray-900/80 to-transparent` maskuje szew z tłem ilustracji (~#020a16). Sprawdzone na 1920 i 1440.
...
- Poprawka na życzenie: siatka 5 kart → karuzela (`'use client'`, scroll-snap, 1/2/3 karty wg szerokości, auto co 5 s z pauzą na hover/touch, strzałki + kropki, aktywna kropka liczona z `scrollLeft`). Logo Google jako inline SVG oficjalnego „G" (marka, nie ikona z zestawu) w karcie oceny, na kartach opinii i w przycisku. Test Playwright: strzałka przełącza na 2/5, pauza działa. Preview env na Vercelu NIE da się dodać z CLI (odrzuca `main`, a „wszystkie gałęzie" wymaga promptu) — tylko Production.
- „Za wielka na tę ilość opinii" → kompaktowy pas `py-8`: siatka `md:grid-cols-[17rem_1fr]`, po lewej h2 `text-base` + G + 4,7 + gwiazdki + linki tekstowe, po prawej karuzela z jedną opinią (`line-clamp-2`), strzałki 28 px. Wysokość 168 px desktop / 316 px mobile (było ~650). Skala sekcji ma odpowiadać liczbie danych: 13 opinii ≠ sekcja bohaterska.
- Ten sam pas na 17 stronach `/serwis-zebra/[miasto]` (strona serwerowa → `async` + `pobierzOpinieGoogle()`), nad sekcją linków stopki. SSR potwierdzony grepem na Warszawie i Krakowie.
...
- Korekta po uwagach użytkownika: lewa scena bez kartonu (zbędny), drukarka NIE drukuje, trzy kontrolki na czerwono (zepsuta); prawa — kurier „jak z InPostu" (żółta kurtka, czapka, bez logo) WYCHODZI i macha, bo drukarka już stoi na biurku po naprawie, obok pusty karton. Wideo wygenerowane ponownie z nowej ilustracji. Logika sceny: kurier przynosi naprawioną drukarkę i wychodzi, nie wchodzi z paczką.
- **Pułapka wideo**: `gemini_omni_flash_1_1` w każdym z 3 wariantów cofał etykietę do drukarki i drukował od nowa, mimo prompta „never retract". Rozwiązanie: obszar drukarki z etykietą (250×330 px przy 612,330 w kadrze 1280×720) nałożony z nieruchomej ilustracji przez `ffmpeg overlay`; pierwszy wycinek był za szeroki i zamroził nogi kuriera. Zasada: elementy, które mają stać nieruchomo, maskować kadrem stillu, nie prosić o to model. Druga poprawka: nakładanie stillu NA film zostawiało pod wycinkiem „ducha" etykiety z filmu — właściwy montaż jest odwrotny: podstawa = still (`loop` + `trim`), z filmu tylko obszary ruchome (drzwi 330×660 @950,0; ekran laptopa 260×180 @370,280). Obszar drzwi musi objąć wystającą rękę kuriera ze stillu, inaczej zostaje żółty ślad na framudze. Wymóg końcowy: bez najechania etykieta wydrukowana (still), po najechaniu drukuje się od pustej szczeliny — obszar drukarki (250×280 @580,405) wzięty z DRUGIEGO filmu (`nasz-serwis-raw2.mp4`) od 1,6 s, gdzie szczelina jest pusta i etykieta wysuwa się bez cofania, z `tpad=clone` na końcu; trzy źródła w jednym `filter_complex`. Wniosek: z kilku nieudanych generacji da się złożyć poprawny klip po regionach, zamiast generować dalej.
...
- „Co naprawiamy": pierwsza wersja z ikonami URZĄDZEŃ (drukarka, terminal, skaner, tablet) odrzucona — użytkownik: ikony mają pokazywać CZYNNOŚCI, nie sprzęt („lipa na maxa"). Druga wersja: etykieta z kodem, paleta z kartonami i ptaszkiem, kod kreskowy z wiązką, trasa z pinezką → `public/icons/line/czynnosc-{etykieta,inwentaryzacja,skanowanie,teren}.png`, 44 px w szarym boxie. Zasada na przyszłość: ikona kategorii = kontekst użycia, nie rysunek produktu.
- Trzecie podejście po pytaniu „co to jest?": paleta i trasa nie czytały się bez podpisu. Wybór użytkownika (pytanie z opcjami): terminale = karton z kodem i wiązką (`czynnosc-kompletacja.png`), tablety = ekran z listą i ptaszkiem (`czynnosc-formularz.png`). Metafory abstrakcyjne (paleta = inwentaryzacja, mapa = teren) nie działają w 44 px — motyw musi zawierać element rozpoznawalny od razu: kod kreskowy, lista, etykieta.
- 6.09: to samo w `components/HowItWorksCity.tsx` (17 stron `/serwis-zebra/[miasto]`) — ikony `krok-*`, czarne numery, neutralne pigułki; z cennika usunięta zielona iskierka przy „Diagnostyka i wycena" (4 karty).
- 6.09: pigułki pod krokami („za mdłe" na szaro) → jasna limonka `bg-[#EDF9C8]` + `text-gray-900 font-semibold`, oba pliki. Uwaga: opacity `/15` nie istnieje w skali Tailwinda, stąd pełny hex.
- 6.09: ikona tabletów uproszczona (ekran z trzema liniami i dużym ptaszkiem w rogu zamiast listy z checkboxami) — gęsta lista nie czytała się w 44 px. Reguła: w tej wielkości najwyżej 3–4 elementy na ikonę.
- Kontener ikon w „Co naprawiamy" ujednolicony z krokami: białe kółko 64 px z obwódką `border-gray-200`, ikona 44 px (użytkownik pokazał kroki jako wzór). Zielone „ptaszki" przy punktach listy zostały (CheckCircle2 z lucide) — do wymiany przy następnym podejściu, jeśli będą razić.
...
- Poprawka wyglądu 5.09 (uwaga użytkownika: „wygląda dramatycznie"): sekcja na stronie głównej miała listy podkreślonych kodów w każdym kafelku i puste kafelki przy klasach z 3 modelami. Teraz kafelki jak na hubie (grafika, nazwa, liczba modeli, zajawka, serie) i jeden indeks modeli pod spodem (klasa → modele po przecinku). Commit 8c06b45 — użytkownik: „jeszcze gorzej". Decyzja (pytanie z opcjami): same 4 kafelki, bez listy modeli — commit a13d065. Linki do 26 kart zostają na hubie, w stopce, na blogu i w instrukcjach. Nauczka: przy uwagach o wyglądzie pytać, nie zgadywać trzeci raz. Ostatecznie sekcja USUNIĘTA (commit 1cc89cd): w sklepie będą jeszcze skanery, tablety i terminale, więc blok sklepu na stronie głównej zaprojektujemy razem z nimi. Linki do kart drukarek: hub, stopka, blog, instrukcje.
...
- Testu lokalnego nie ma (MAIL_IMAP_PASSWORD na Vercelu jest poufne, pull zwraca pusty string). **Zweryfikowane na produkcji**: dwa oznaczone pytania przez `/api/device-enquiry` → w logach `[mail] SMTP cyber_folks → serwis@takma.com.pl`, w Resend brak wpisu. Logi czyta się strumieniem `vercel logs <url>` uruchomionym PRZED wysyłką (REST runtime-logs i MCP Vercel nie działają dla tego zespołu).
