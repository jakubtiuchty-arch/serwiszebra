...
- Pasek statystyk („to też mi się nie podoba") → `components/LicznikiStat.tsx`: `ul` z `divide-x` (desktop) / `divide-y` (telefon), pole = ikona 28 px w kółku 48 px (certyfikat, karton, lupa z kluczem, uścisk dłoni) + liczba `text-3xl tabular-nums` odliczana od zera (`requestAnimationFrame`, 1,8 s, ease-out sześcienny, `Intl.NumberFormat('pl-PL')`) + sufiks „+"/„lat" w limonce + podpis. Dane jako `{liczba, sufiks, label, icon}`. Sekcja `py-4 sm:py-6` — niższa niż była.
